if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  function actualizarTotalSueldos() {
    ...
    $.ajax({
      url: "/produccionuva2/produccionuva2_sueldo_mensual/?year=" + year,
      type: "GET",
      dataType: "json",
  ...
  function actualizarTotalSalarios() {
    const year = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
    $.ajax({
      url: "/produccionuva2/produccionuva2_salario_mensual/?year=" + year,
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
  ...
  function initPresupuestoSueldos() {
    obtenerTasaCargaSocial().then(function () {
      table = $("#sueldosTable").DataTable({
        ...
        ajax: {
          url: "/produccionuva2/produccionuva2_sueldos_dl/",
          dataSrc: "data",
        },
        columns: [
        ...
        footerCallback: function (row, data, start, end, display) {
          ...
          $.ajax({
            url: "/produccionuva2/produccionuva2_sueldo_mensual/",
            type: "GET",
            success: function (response) {
    ...
    $("#formSueldoSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/produccionuva2/produccionuva2_sueldos_dl/${formId}/`
            : "/produccionuva2/produccionuva2_sueldos_dl/",
  ...
  function initPresupuestoSalarios() {
    obtenerTasaCargaSocial_salarios().then(function () {
      tableSalarios = $("#salariosTable").DataTable({
        ...
        ajax: {
          url: "/produccionuva2/produccionuva2_salarios_dl/",
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
        ...
        footerCallback: function (row, data, start, end, display) {
          ...
          $.ajax({
            url: "/produccionuva2/produccionuva2_salario_mensual/",
            type: "GET",
            success: function (response) {
              if (response.data && response.data.length > 0) {
                // Crear un mapa de los consolidados por mes
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/produccionuva2/produccionuva2_salarios_dl/${formId}/`
            : "/produccionuva2/produccionuva2_salarios_dl/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
  ...
  function eliminarSueldo(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/produccionuva2/produccionuva2_sueldos_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
  ...
  function eliminarSalario(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/produccionuva2/produccionuva2_salarios_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
              Swal.fire("¡Eliminado!", response.message, "success");
