...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/produccionuva2/produccionuva2_servicios_dl/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          // Recargar la tabla
          dt.ajax.url("/produccionuva2/produccionuva2_servicios_dl/?year=" + yearVal).load(function () {
    ...
    ajax: {
      url: "/produccionuva2/produccionuva2_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/produccionuva2/produccionuva2_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionuva2/produccionuva2_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/produccionuva2/produccionuva2_servicios_dl/${id}/`
      : "/produccionuva2/produccionuva2_servicios_dl/",
