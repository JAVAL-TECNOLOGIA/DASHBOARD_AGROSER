...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/servicios/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/servicios/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/servicios/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/api_servicios_totales/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
