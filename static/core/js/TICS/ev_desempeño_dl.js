...
// Función para crear el elemento de una pregunta individual
/* function crearElementoPregunta(pregunta) {
  ...
  radios.forEach((radio) => {
    radio.addEventListener("change", function () {
      // Actualizar cálculo de promedios según nueva escala
      const valorMap = {
        baja: 1,
        aceptable: 3,
        alta: 5,
      ...
      actualizarPromedioCategoria(pregunta.categoria, valorMap[this.value]);
    });
  });
  ...
  var table = $("#tablaEvaluaciones").DataTable({
    ...
    buttons: [
      ...
      },
      {
        ...
        text: '<i class="fas fa-plus-circle"></i> Nueva Evaluación',
...
async function cargarEvaluacion(id) {
  try {
    console.log("Intentando cargar evaluación con ID:", id); // Debug: confirmar ID
