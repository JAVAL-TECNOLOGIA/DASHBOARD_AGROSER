...
function initServiciosTable() {
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    ajax: {
      url: "/logistica/logistica_servicios_dl/?year=" + yearVal,
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/logistica/logistica_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/logistica/logistica_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/logistica/logistica_servicios_dl/${id}/`
      : "/logistica/logistica_servicios_dl/",
