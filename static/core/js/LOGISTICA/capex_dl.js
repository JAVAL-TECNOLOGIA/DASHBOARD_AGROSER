  ...
  $(descripcionSelector)
    .autocomplete({
      ...
      select: function (event, ui) {
        ...
        $(descripcionSelector).val(ui.item.label).trigger('change');
...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/logistica/logistica_capex/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/logistica/logistica_capex/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/logistica/logistica_capex/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/logistica/logistica_capex/" + formId + "/" : "/logistica/logistica_capex/",
 ...
 */
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/logistica/logistica_capex/" + formId + "/"
        : "/logistica/logistica_capex/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function editarProducto_capex(id) {
  $.ajax({
    url: "/logistica/logistica_capex/" + id + "/",
    type: "GET",
    success: function (data) {
