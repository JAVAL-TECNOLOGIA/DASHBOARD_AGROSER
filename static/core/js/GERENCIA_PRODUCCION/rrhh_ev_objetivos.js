...
function inicializarTablaEvaluaciones_objetivos() {
  var table = $("#tablaEvaluaciones_objetivos").DataTable({
   ...
   ajax: {
      url: "/gerencia_produccion/objetivos_evaluacion/",
      type: "GET",
      data: function (d) {
          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerGereProd.generarEnviarPDFEvaluacion(${row.id})" 
...
function inicializarTablaDetallesObjetivos(id_evaluacion) {
  var table = $("#tablaDetallesObjetivos").DataTable({
    ...
    ajax: {
      url: "/gerencia_produccion/detalles_objetivos/",
      type: "GET",
...
function guardarObjetivoYSiguiente() {
  ...
  const datosEvaluacion = {
    ...
    periodo: $("#periodo_objetivos").val(), // Ahora es un input, no un select
  ...
  $.ajax({
    url: "/gerencia_produccion/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
    success: function (response) {
      if (response.status === "success") {
        ...
        // Buscar si existe una evaluación para este evaluado en el mismo periodo
        ...
        if (evaluacionExistente) {
          // Ya existe una evaluación para este periodo, preguntamos si quiere continuar a la parte de competencias
          Swal.fire({
            icon: "info",
        ...
        // Si no existe evaluación para este periodo, continuar con el proceso de guardado
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/gerencia_produccion/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/gerencia_produccion/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
//########################################################
//MÓDULO DE GESTIÓN DE CORREOS ELECTRÓNICOS Y PDFs - ALMACÉN
//########################################################

const EmailManagerGereProd = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/gerencia_produccion/objetivos_evaluacion/",
      detallesObjetivos: "/gerencia_produccion/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/GERENCIA_PRODUCCION/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - GERENCIA PRODUCCION",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de GERENCIA PRODUCCION?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - GERENCIA PRODUCCION",
        text: "Generando PDF desde GERENCIA PRODUCCION y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en GERENCIA PRODUCCION",
    ...
    metadata: {
      modulo: "GERENCIA PRODUCCION",
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - GERENCIA PRODUCCION",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar el módulo de gestión de correos electrónicos para RIEGO
  EmailManagerGereProd.inicializar();
