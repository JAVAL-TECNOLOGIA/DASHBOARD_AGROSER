...
function initServiciosTable() {
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/produccionuva1/produccionuva1_servicios_cv/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/produccionuva1/produccionuva1_servicios_cv/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/produccionuva1/produccionuva1_servicios_totals_cv/?year=${yearVal}`,
    type: "GET",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionuva1/produccionuva1_servicios_cv/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/produccionuva1/produccionuva1_servicios_cv/${id}/`
      : "/produccionuva1/produccionuva1_servicios_cv/",
