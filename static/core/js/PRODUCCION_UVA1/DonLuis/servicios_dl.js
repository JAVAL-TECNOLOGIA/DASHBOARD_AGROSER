...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/produccionuva1/produccionuva1_servicios_dl/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          // Obtener el año actual del filtro
          var currentYear = $('#filtroAnioPresupuesto').val() || $('#cmbCampania').val() || 'CAMP' + new Date().getFullYear();

          // Recargar la tabla con el año correcto
          dt.ajax.url("/produccionuva1/produccionuva1_servicios_dl/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/produccionuva1/produccionuva1_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || $('#cmbCampania').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    ...
    // Agregar la configuración del footer
    /* footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(7).footer()).html("<strong> TOTALES: </strong>");
  
        // Cargar los totales desde la API
        $.ajax({
          url: "/costos/costo_servicios_totals_dl/",
          type: "GET",
          async: false, // Importante para asegurar que los totales se carguen antes de continuar
          success: function (response) {
            ...
            meses.forEach((mes, index) => {
              // Calcular índices para cantidad y precio
              const cantidadIndex = 7 + index * 2;
              const precioIndex = cantidadIndex + 1;
  
              // Obtener valores de la API
              const cantidadKey = `Total_${mes}_Cantidad`;
              const precioKey = `Total_${mes}_Precio`;
  
              // Actualizar footer con los totales
              $(api.column(cantidadIndex).footer()).html(`
                  <div class="text-center">
                    ${response[cantidadKey] || 0}
                  </div>
                `);
  
              $(api.column(precioIndex).footer()).html(`
                  <div class="text-right">
                    S/. ${(response[precioKey] || 0).toFixed(2)}
                  </div>
                `);
            });
  
            // Actualizar el total general (penúltima columna)
            const totalGeneralIndex = api.columns().nodes().length - 2;
            $(api.column(totalGeneralIndex).footer()).html(`
                <div class="text-right">
                  <strong>S/. ${(response.Total_General_Precio || 0).toFixed(
                    2
                  )}</strong>
                </div>
              `);
  
            // Actualizar el widget de stats_serviciosterceros con el total general
            $("#stats_serviciosterceros").html(
              `S/. ${(response.Total_General_Precio || 0).toFixed(2)}`
            );
            $(api.column(-1).footer()).html("");
          },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/produccionuva1/produccionuva1_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionuva1/produccionuva1_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/produccionuva1/produccionuva1_servicios_dl/${id}/`
      : "/produccionuva1/produccionuva1_servicios_dl/",
