          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerUva2.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/produccionuva2/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionuva2/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/produccionuva2/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerUva2 = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/produccionuva2/objetivos_evaluacion/",
      detallesObjetivos: "/produccionuva2/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/PRODUCCION_UVA2/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - PRODUCCIÓN UVA2",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de PRODUCCIÓN UVA2?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - PRODUCCIÓN UVA2",
        text: "Generando PDF desde PRODUCCIÓN UVA2 y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en PRODUCCIÓN UVA2",
    ...
    metadata: {
      modulo: "PRODUCCIÓN UVA2",
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - PRODUCCIÓN UVA2",
    },
  },
