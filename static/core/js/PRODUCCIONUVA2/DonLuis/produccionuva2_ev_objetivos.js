...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/tiv/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
/**
 * Módulo para manejar la funcionalidad de envío de correos y generación de PDFs
 * Especializado para el módulo de TIC con configuración específica
 */
const EmailManager = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/objetivos_evaluacion/",
      detallesObjetivos: "/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/produccionuva2/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - PRODUCCION UVA 2",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de PRODUCCION UVA 2?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - PRODUCCION UVA 2",
        text: "Generando PDF desde PRODUCCION UVA 2 y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en PRODUCCION UVA 2",
    ...
    metadata: {
      modulo: "PRODUCCION UVA 2",
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - PRODUCCION UVA 2",
    },
  },
  ...
  generarEnviarPDFEvaluacion(idEvaluacion) {
    // Log específico para Almacén
    console.log(
      `[Almacén] Iniciando proceso de envío de PDF para evaluación ID: ${idEvaluacion}`
    );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar el módulo de gestión de correos electrónicos para TIC
