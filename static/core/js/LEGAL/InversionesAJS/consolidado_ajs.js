...
async function obtenerConsolidadoSueldos() {
  try {
    const responseSueldos = await fetchDataConReintento(
      "/legal/legal_sueldo_mensual_ajs/", // Ajusta si la ruta difiere
      3,
      2000
    );
...
async function obtenerConsolidadoSalarios() {
  try {
    const responseSalarios = await fetchDataConReintento(
      "/legal/legal_salario_mensual_ajs/", // Ojo con la ruta
      3,
      2000
    );
...
async function obtenerConsolidadoSueldosYSalarios() {
  const sueldos = await obtenerConsolidadoSueldos();
  const salarios = await obtenerConsolidadoSalarios();

  const total = sueldos + salarios;
  $("#stats_consolidado_sueldos").text(formatoSoles.format(total));
  return total;
}
