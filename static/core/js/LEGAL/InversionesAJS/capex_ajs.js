...
function calcularTotalPresupuesto_Capex() {
  $.ajax({
    url: "/legal/legal_capex_totals_ajs/",
    type: "GET",
    dataType: "json",
    success: function (data) {
    ...
    error: function (xhr, status, error) {
      Swal.fire({
        title: "Error!",
        text: "Error al obtener el total de los suministros de CAPEX",
        icon: "error",
      });
    },
  });
...
function initCapexTable() {
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "legal/legal_capex_ajs");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/legal/legal_capex_ajs/" + formId + "/"
        : "/legal/legal_capex_ajs/",
