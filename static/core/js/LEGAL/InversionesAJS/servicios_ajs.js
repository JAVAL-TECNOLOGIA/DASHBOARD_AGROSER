...
function initServiciosTable() {
   ...
   */
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      // Cargar los totales desde la API
      $.ajax({
        url: "/legal/legal_servicios_totals_ajs/",
        type: "GET",
        success: function (response) {
          const meses = [
            "Enero",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/legal/legal_servicios_ajs/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/legal/legal_servicios_ajs/${id}/`
      : "/legal/legal_servicios_ajs/",
