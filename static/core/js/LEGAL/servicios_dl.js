...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/legal/legal_servicios_dl/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          // Obtener el año actual del filtro antes de recargar
          var yearVal = $('#filtroAnioPresupuesto').val() || $('#cmbCampania').val() || 'CAMP' + new Date().getFullYear();
          
          // Recargar la tabla con el parámetro year
          dt.ajax.url("/legal/legal_servicios_dl/?year=" + yearVal).load(function () {
    ...
    ajax: {
      url: "/legal/legal_servicios_dl/?year=" + yearVal,
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/legal/legal_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/legal/legal_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/legal/legal_servicios_dl/${id}/`
      : "/legal/legal_servicios_dl/",
