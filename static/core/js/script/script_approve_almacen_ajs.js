...
// =============================================
function AJPCvalidarPermisosPorCargo(idCargo) {
...
// =============================================
function AJPCobtenerUrlConEstados(area) {
  if (!permisoValidado || estadosPermitidos.length === 0) {
    // Si no hay permisos, retornar URL que no devuelva datos
    return "/approve_almacen_log_filter_ajs/" + area + "/SINPERMISOS/";
  }

  // Para almacén solo manejamos estado PE (PENDIENTE)
  return "/approve_almacen_log_filter_ajs/" + area + "/";
}

// =============================================
// FUNCIÓN PARA CARGAR ÁREAS
// =============================================
function AJPCcargarAreas() {
  $.get("/approve_almacen_areas", function (data) {
    $("#AJPCselectApproveArea").append("<option value='0'>Todos</option>");
    $.each(data, function (index, value) {
      $("#AJPCselectApproveArea").append(
        "<option value='" +
          value["idarea"] +
          "'>" +
          value["descripcion"] +
...
function AJPCinicializarTabla() {
  ...
  $("#AJPCpanelSeleccion").hide();
  $("#AJPCbtnAprobacionMasiva").prop("disabled", true);

  // Destruir la tabla existente si existe
  if ($.fn.DataTable.isDataTable("#AJPCtableApproveOrders")) {
    $("#AJPCtableApproveOrders").DataTable().clear().destroy();
    $("#AJPCtableApproveOrders tbody").empty();
  }

  var urlConEstados = AJPCobtenerUrlConEstados(0); // 0 = Todas las áreas

  console.log("📊 Inicializando tabla con URL:", urlConEstados);

  table_show = $("#AJPCtableApproveOrders").DataTable({
    ajax: {
      url: urlConEstados,
      dataSrc: "",
    },
      ...
      { data: "area" }, // 7 - Área
      {
        ...
        render: function (data, type, row) {
          return (
            ...
            '<input type="checkbox" class="form-check-input row-checkbox-PCAJS" value="' +
            row.idorden +
            '" data-estado="' +
            row.estado +
            '">' +
...
function AJPCfiltrarPorArea() {
  ...
  var area = $("#AJPCselectApproveArea").val() || 0;
  ...
  var urlConEstados = AJPCobtenerUrlConEstados(area);
...
// =============================================
function AJPCconfigurarEventos() {
  // Remover event listeners anteriores para evitar duplicación
  $("#AJPCbtnApproveFilter").off("click");
  $("#AJPCbtnAprobacionMasiva").off("click");
  $("#AJPCbtnSeleccionarTodos").off("click");
  $("#AJPCbtnDeseleccionarTodos").off("click");

  // Evento del botón filtrar
  $("#AJPCbtnApproveFilter").click(function () {
    AJPCfiltrarPorArea();
  });

  // Evento del botón de aprobación masiva
  $("#AJPCbtnAprobacionMasiva").click(function () {
    AJPCejecutarAprobacionMasiva();
  });

  // Eventos de los botones del panel de selección
  $("#AJPCbtnSeleccionarTodos").click(function () {
    AJPCmanejarSeleccionTodos(true);
  });

  $("#AJPCbtnDeseleccionarTodos").click(function () {
    AJPCmanejarSeleccionTodos(false);
  });
}
...
// =============================================
function AJPCconfigurarSeleccionFila() {
  // Remover event listeners anteriores para evitar duplicación
  $("#AJPCtableApproveOrders tbody").off("dblclick");

  $("#AJPCtableApproveOrders tbody").on("dblclick", "tr", function (e) {
    ...
    $("#AJPClblEstado").text(estadoFila);
    $("#AJPClblArea").text("-");
    $("#AJPClblResponsable").text("-");
    $("#AJPClblObservacion").text("-");
    $("#AJPClblTotal").text("N/A");
    $("#AJPCtableBodyApproveOrdersLogDetail").html("");
    ...
    setTimeout(function () {
      ...
      if (
        ...
        $("#AJPCbuttons-loadings").addClass("d-none");
        $("#AJPCbtn-aprobar").removeClass("d-none");
      } else {
        $("#AJPCbuttons-loadings").addClass("d-none");
        $("#AJPCbtn-aprobar").addClass("d-none");
      }
    }, 50);

    // Remover event listeners anteriores de los botones para evitar duplicación
    $("#AJPCbtn-aprobar").off("click");

    if (documento == "COMPRA") {
      AJPCabrirModalCompra(idorden);
    } else if (documento == "SERVICIO") {
      AJPCabrirModalServicio(idorden);
    }

    // Configurar botones del modal
    AJPCconfigurarBotonesModal(idorden);
  });
}
...
// =============================================
function AJPCabrirModalCompra(idorden) {
  $("#AJPCmodalApproveOrders").modal("toggle");
  $.get(
    "/approve_almacen_log_detail_purchase_ajs/" + idorden + "/",
    function (data) {
      const first = data && data.length ? data[0] : {};
      $("#AJPClblArea").html(first["area"] || "-");
      $("#AJPClblObservacion").html(first["observacion"] || "-");
      // Solo actualizar el estado si viene del backend, sino mantener el prefijado
      if (first["estado"]) {
        $("#AJPClblEstado").html(first["estado"]);
      }
      $("#AJPClblResponsable").html(first["responsable"] || "-");
      $("#AJPClblTotal").html("N/A");
      $("#AJPCtableBodyApproveOrdersLogDetail").html("");
      $.each(data || [], function (index, value) {
        $("#AJPCtableBodyApproveOrdersLogDetail").append(
          "<tr><td>" +
            (value["idproducto"] || "-") +
            "</td><td>" +
            (value["producto"] || "-") +
...
// =============================================
function AJPCabrirModalServicio(idorden) {
  $("#AJPCmodalApproveOrders").modal("toggle");
  $.get("/approve_orders_log_detail_service/" + idorden, function (data) {
    $("#AJPClblArea").html(data[0]["area"]);
    $("#AJPClblEstado").html(data[0]["estado"]);
    $("#AJPClblObservacion").html(data[0]["observacion"]);
    $("#AJPClblResponsable").html(data[0]["responsable"]);
    $("#AJPClblTotal").html(data[0]["total_os"]);
    $("#AJPCtableBodyApproveOrdersLogDetail").html("");
    $.each(data, function (index, value) {
      $("#AJPCtableApproveOrdersLogDetail").append(
        "<tr><td>" +
          value["idproducto"] +
          "</td><td>" +
          value["producto"] +
...
// =============================================
function AJPCconfigurarBotonesModal(idorden) {
  $("#AJPCbtn-aprobar").click(function () {
    AJPCactualizarOrden("aprobar", idorden);
  });
}
...
// =============================================
function AJPCactualizarOrden(accion, idorden) {
  $.ajax({
    url: "/actualizar-orden-almacen-ajs/" + idorden + "/",
    type: "GET",
    data: { accion: accion },
    ...
    success: function (data, textStatus, xhr) {
      ...
      AJPCinicializarTabla();
      $("#AJPCmodalApproveOrders").modal("hide");
    },
...
// =============================================
function AJPCextraerDatosUsuario() {
  let usuario = $("#AJPCUser").val();
  let userId = $("#AJPCUserId").val();

  console.log("Usuario logueado:", usuario);
  console.log("AJPCUser ID logueado:", userId);
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (
        ...
        if (AJPCvalidarPermisosPorCargo(datosUsuario.IDCARGO)) {
...
function AJPCactualizarContadorSeleccion() {
  var cantidad = registrosSeleccionados.length;
  $("#AJPCcontadorSeleccion").text(
    cantidad +
      " registro" +
      (cantidad !== 1 ? "s" : "") +
      " seleccionado" +
  ...
  if (cantidad > 0) {
    $("#AJPCpanelSeleccion").slideDown(300);
    $("#AJPCbtnAprobacionMasiva").prop("disabled", false);
  } else {
    $("#AJPCpanelSeleccion").slideUp(300);
    $("#AJPCbtnAprobacionMasiva").prop("disabled", true);
    estadoActualSeleccionado = null;
  }
}
...
function AJPCactualizarTextoBtnMasivo(estado) {
  var texto = "";
  var icono = "";
  ...
  if (estado === "PE") {
    ...
    $("#AJPCbtnAprobacionMasiva")
      .removeClass("btn-warning")
      .addClass("btn-success");
  }

  $("#AJPCtextoBtnMasivo").text(texto);
  $("#AJPCbtnAprobacionMasiva i")
    .removeClass()
    .addClass("fa " + icono + " mr-1");
}

// Función para manejar selección individual
function AJPCmanejarSeleccionIndividual(checkbox) {
  ...
  if ($checkbox.is(":checked")) {
    ...
    if (
      ...
      if (estadoActualSeleccionado === null) {
        estadoActualSeleccionado = estado;
        AJPCactualizarTextoBtnMasivo(estado);
      }
    } else {
  ...
  var totalCheckboxes = $(".row-checkbox-PCAJS").length;
  var checkedCheckboxes = $(".row-checkbox-PCAJS:checked").length;

  if (checkedCheckboxes === 0) {
    $("#AJPCcheckboxSelectAll").prop("checked", false).prop("indeterminate", false);
  } else if (checkedCheckboxes === totalCheckboxes) {
    $("#AJPCcheckboxSelectAll").prop("checked", true).prop("indeterminate", false);
  } else {
    $("#AJPCcheckboxSelectAll").prop("checked", false).prop("indeterminate", true);
  }

  AJPCactualizarContadorSeleccion();
}

// Función para seleccionar/deseleccionar todos
function AJPCmanejarSeleccionTodos(seleccionar) {
  registrosSeleccionados = [];
  estadoActualSeleccionado = null;

  $(".row-checkbox-PCAJS").each(function () {
    var $checkbox = $(this);
    var $row = $checkbox.closest("tr");
...
function AJPCejecutarAprobacionMasiva() {
  if (registrosSeleccionados.length === 0) {
    Swal.fire({
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      AJPCprocesarAprobacionMasiva(accion);
    }
  });
}

// Función para procesar la aprobación masiva
function AJPCprocesarAprobacionMasiva(accion) {
  ...
  var promesas = registrosSeleccionados.map(function (registro, index) {
    return new Promise(function (resolve) {
      setTimeout(function () {
        $.ajax({
          url: "/actualizar-orden-almacen-ajs/" + registro.idorden + "/",
          type: "GET",
          data: { accion: accion },
          timeout: 10000,
  ...
  Promise.all(promesas).then(function (resultados) {
    ...
    $("#AJPCpanelSeleccion").hide();
    $("#AJPCbtnAprobacionMasiva").prop("disabled", true);
    $(".row-checkbox-PCAJS").prop("checked", false);
    $("#AJPCcheckboxSelectAll").prop("checked", false).prop("indeterminate", false);
...
// =============================================
function AJPCconfigurarEventosCheckbox() {
  // Eventos para checkboxes individuales
  $(".row-checkbox-PCAJS")
    .off("change")
    .on("change", function () {
      AJPCmanejarSeleccionIndividual(this);
    });
  ...
  $("#AJPCcheckboxSelectAll")
    ...
    .on("change", function () {
      ...
      AJPCmanejarSeleccionTodos(isChecked);
    });
}
...
// =============================================
window.initPComprasAJSModule = function (scope) {

   const $scope = $(scope || document);
  // 2) evita duplicar handlers si cambias de tab
  $(document).off("show.bs.modal", "#AJPCmodalApproveOrders");
  $(document).off("hide.bs.modal", "#AJPCmodalApproveOrders");

  // 3) MutationObserver para AJPClblEstado
  const observerEstado = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      if (mutation.target.id === "AJPClblEstado") {
        ...
        console.log(' -1 AJPCmodalApproveOrders show.bs.modal');
      }
    });
  });
  // 4) Evento al abrir modal
  $(document).on("show.bs.modal", "#AJPCmodalApproveOrders", function () {
    $("#AJPCbuttons-loadings").removeClass("d-none");
    $("#AJPCbtn-aprobar").addClass("d-none");

    const elementoEstado = document.getElementById("AJPClblEstado");
    if (elementoEstado) {
 ...
 $(document).on("hide.bs.modal", "#AJPCmodalApproveOrders", function () {
  observerEstado.disconnect();
   ...
   AJPCconfigurarEventos();
   AJPCconfigurarSeleccionFila();

   // Carga inicial de tabla (usa filtro por área por defecto)
   //PCAJPCfiltrarPorArea();
   AJPCextraerDatosUsuario();
};