...
// =============================================
function AJOCvalidarPermisosPorCargo(idCargo) {
...
function AJOCobtenerUrlConEstados(area) {
  ...
  console.log(
    ...
    area + ":",
    "/approve_compras_log_filter_ajs/" + area + "/" + estadosStr + "/"
  );
  console.log("📋 Estados permitidos:", estadosPermitidos);
  console.log("🔄 Estados mapeados:", estadosMapeados);

  return "/approve_compras_log_filter_ajs/" + area + "/" + estadosStr + "/";
}

// =============================================
// FUNCIÓN PARA CARGAR ÁREAS
// =============================================
function AJOCcargarAreas() {
  $.get("/approve_orders_areas", function (data) {
    $("#AJOCselectApproveArea").append("<option value='0'>Todos</option>");
    /* console.log($("#AJOCselectApproveArea").val()); */
    $.each(data, function (index, value) {
      $("#AJOCselectApproveArea").append(
        "<option value='" +
          value["idarea"] +
          "'>" +
          value["descripcion"] +
...
function AJOCinicializarTabla() {
  ...
  $("#AJOCpanelSeleccion").hide();
  $("#AJOCbtnAprobacionMasiva").prop("disabled", true);

  // Destruir la tabla existente si existe
  if ($.fn.DataTable.isDataTable("#AJOCtableApproveOrders")) {
    $("#AJOCtableApproveOrders").DataTable().clear().destroy();
    $("#AJOCtableApproveOrders tbody").empty();
  }

  var urlConEstados = AJOCobtenerUrlConEstados(0); // 0 = Todas las áreas

  console.log("🚀 Inicializando tabla con URL:", urlConEstados);

  table_show = $("#AJOCtableApproveOrders").DataTable({
    ajax: {
      url: urlConEstados,
      dataSrc: "",
    },
      ...
      { data: "total" },
      {
        ...
        render: function (data, type, row) {
          return (
            ...
            '<input type="checkbox" class="form-check-input row-checkbox-OCAJS" value="' +
            row.idorden +
            '" data-estado="' +
            row.estado +
            '">' +
...
function AJOCfiltrarPorArea() {
  ...
  var area = $("#AJOCselectApproveArea").val() || 0;
  ...
  var urlConEstados = AJOCobtenerUrlConEstados(area);
...
// =============================================
function AJOCconfigurarEventos() {
  // Remover event listeners anteriores para evitar duplicación
  $("#AJOCbtnApproveShow").off("click");
  $("#AJOCbtnAprobacionMasiva").off("click");
  $("#AJOCbtnSeleccionarTodos").off("click");
  $("#AJOCbtnDeseleccionarTodos").off("click");

  // Evento del botón filtrar
  $("#AJOCbtnApproveShow").click(function () {
    AJOCfiltrarPorArea();
  });

  // Evento del botón de aprobación masiva
  $("#AJOCbtnAprobacionMasiva").click(function () {
    AJOCejecutarAprobacionMasiva();
  });

  // Eventos de los botones del panel de selección
  $("#AJOCbtnSeleccionarTodos").click(function () {
    AJOCmanejarSeleccionTodos(true);
  });

  $("#AJOCbtnDeseleccionarTodos").click(function () {
    AJOCmanejarSeleccionTodos(false);
  });
}
// =============================================
// FUNCIÓN PARA MANEJAR DOBLE CLICK EN FILAS
// =============================================
function AJOCconfigurarSeleccionFila() {
  // Remover event listeners anteriores para evitar duplicación
  $("#AJOCtableApproveOrders tbody").off("dblclick");

  $("#AJOCtableApproveOrders tbody").on("dblclick", "tr", function (e) {
    ...
    if (row["documento"] == "COMPRA") {
      AJOCabrirModalCompra(idservicio);
    } else if (row["documento"] == "SERVICIO") {
      AJOCabrirModalServicio(idservicio);
    }

    // Configurar botones del modal
    AJOCconfigurarBotonesModal(idservicio);
  });
}
...
// =============================================
function AJOCabrirModalCompra(idorden) {
  $("#AJOCmodalApproveOrders").modal("toggle");
  $.get(
    "/approve_orders_log_detail_purchase_ajs/" + idorden + "/",
    function (data) {
      $("#AJOClblEstado").html(data[0]["estado"]);
      $("#AJOClblProveedor").html(data[0]["proveedor"]);
      $("#AJOClblRuc").html(data[0]["ruc"]);
      $("#AJOClblCondicion").html(data[0]["formapago"]);
      $("#AJOClblResponsable").html(data[0]["responsable"]);
      $("#AJOClblArea").html(data[0]["area"]);
      $("#AJOClblSerieNumero").html(data[0]["SerieNumero"]);
      $("#AJOClblTotal").html(data[0]["total_oc"]);
      $("#AJOCtableBodyApproveOrdersLogDetail").html("");

      $.each(data, function (index, value) {
        //total = total +(Math.round(value['total'] * 100) / 100);
        $("#AJOCtableApproveOrdersLogDetail").append(
          "<tr><td>" +
            value["producto"] +
            "</td><td>" +
            value["idmedida"] +
...
// =============================================
function AJOCabrirModalServicio(idorden) {
  $("#AJOCmodalApproveOrders").modal("toggle");
  $.get("/approve_orders_log_detail_service/" + idorden, function (data) {
    $("#AJOClblEstado").html(data[0]["estado"].toUpperCase());
    $("#AJOClblResponsable").html(data[0]["responsable"]);
    $("#AJOClblProveedor").html(data[0]["proveedor"]);
    $("#AJOClblCondicion").html(data[0]["condicion"]);
    $("#AJOClblSerieNumero").html(data[0]["serie_numero"]);
    $("#AJOClblArea").html(data[0]["area"]);
    $("#AJOClblRuc").html(data[0]["ruc"]);
    $("#AJOCAJOClblTotal").html(data[0]["total_os"]);
    $("#AJOCtableBodyApproveOrdersLogDetail").html("");
...
// =============================================
function AJOCconfigurarBotonesModal(idservicio) {
  $("#AJOCbtn-aprobar")
    .off("click")
    .click(function () {
      AJOCactualizarOrden("aprobar", idservicio);
    });

  $("#AJOCbtn-vb")
    .off("click")
    .click(function () {
      AJOCactualizarOrden("vb", idservicio);
    });

  $("#AJOCbtn-anular")
    .off("click")
    .click(function () {
      AJOCactualizarOrden("anular", idservicio);
    });
}
// =============================================
// FUNCIÓN PARA ACTUALIZAR ORDEN
// =============================================
function AJOCactualizarOrden(accion, idservicio) {
  $.ajax({
    url: "/actualizar-orden-compra-ajs/" + idservicio + "/",
    type: "GET",
    data: { accion: accion },
    timeout: 15000,
    success: function (data, textStatus, xhr) {
      ...
      Swal.fire({
        ...
        $("#AJOCmodalApproveOrders").modal("hide");
        AJOCfiltrarPorArea(); // Recargar la tabla con los filtros actuales
      });
    },
...
// =============================================
function AJOCextraerDatosUsuario() {
  let usuario = $("#AJOCUser").val();
  let userId = $("#AJOCUserId").val();

  console.log("👤 Usuario extraído del DOM:", usuario);
  console.log("🆔 AJOCUserId extraído del DOM:", userId);
    ...
    success: function (response) {
      ...
      if (
        ...
        if (usuarioEncontrado) {
          ...
          const tienePermisos = AJOCvalidarPermisosPorCargo(
...
function AJOCactualizarContadorSeleccion() {
  var cantidad = registrosSeleccionados.length;
  $("#AJOCcontadorSeleccion").text(
    cantidad +
      " registro" +
      (cantidad !== 1 ? "s" : "") +
      " seleccionado" +
  ...
  if (cantidad > 0) {
    $("#AJOCpanelSeleccion").show();
    $("#AJOCbtnAprobacionMasiva").prop("disabled", false);
  } else {
    $("#AJOCpanelSeleccion").hide();
    $("#AJOCbtnAprobacionMasiva").prop("disabled", true);

    // Resetear el botón a su estado original cuando no hay selecciones
    $("#AJOCtextoBtnMasivo").text("Aprobar Masivo");
    $("#AJOCbtnAprobacionMasiva i")
      .removeClass()
      .addClass("fa fa-check-circle mr-1");
    $("#AJOCbtnAprobacionMasiva")
...
function AJOCactualizarTextoBtnMasivo(estado) {
  var texto = "";
  var icono = "";
...
function AJOCmanejarSeleccionIndividual(checkbox) {
  ...
  if ($checkbox.is(":checked")) {
    ...
    if (registrosSeleccionados.length === 0) {
      ...
      AJOCactualizarTextoBtnMasivo(estado);
  ...
  var totalCheckboxes = $(".row-checkbox-OCAJS").length;
  var checkedCheckboxes = $(".row-checkbox-OCAJS:checked").length;

  if (checkedCheckboxes === 0) {
    $("#AJOCcheckboxSelectAll").prop("checked", false).prop("indeterminate", false);
  } else if (checkedCheckboxes === totalCheckboxes) {
    $("#AJOCcheckboxSelectAll").prop("checked", true).prop("indeterminate", false);
  } else {
    $("#AJOCcheckboxSelectAll").prop("checked", false).prop("indeterminate", true);
  }

  AJOCactualizarContadorSeleccion();
}

// Función para seleccionar/deseleccionar todos
function AJOCmanejarSeleccionTodos(seleccionar) {
  registrosSeleccionados = [];
  estadoActualSeleccionado = null;

  $(".row-checkbox-OCAJS").each(function () {
    var $checkbox = $(this);
    var estado = $checkbox.data("estado");
    var idorden = $checkbox.val();
    var $row = $checkbox.closest("tr");
...
function AJOCejecutarAprobacionMasiva() {
  if (registrosSeleccionados.length === 0) {
    Swal.fire({
...
function AJOCprocesarAprobacionMasiva(accion) {
  ...
  var promesas = registrosSeleccionados.map(function (registro, index) {
    return new Promise(function (resolve) {
      setTimeout(function () {
        $.ajax({
          url: "/actualizar-orden-compra-ajs/" + registro.idorden + "/",
          type: "GET",
          data: { accion: accion },
          timeout: 10000,
  ...
  Promise.all(promesas).then(function (resultados) {
    ...
    Swal.fire({
      ...
      AJOCfiltrarPorArea(); // Recargar la tabla
    });
  });
}
...
// =============================================
function AJOCconfigurarEventosCheckbox() {
  // Eventos para checkboxes individuales
  $(".row-checkbox-OCAJS")
    .off("change")
    .on("change", function () {
      AJOCmanejarSeleccionIndividual(this);
    });
  ...
  $("#AJOCcheckboxSelectAll")
    ...
    .on("change", function () {
      ...
      AJOCmanejarSeleccionTodos(isChecked);
    });
}
...
// =============================================
window.initOComprasAJSModule = function (scope) {
      const $scope = $(scope || document);
  // 2) evita duplicar handlers si cambias de tab
  $(document).off("show.bs.modal", "#AJOCmodalApproveOrders");
  $(document).off("hide.bs.modal", "#AJOCmodalApproveOrders");

  // 3) MutationObserver para OCOCVAJOClblEstado
  const observerEstado = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      if (mutation.target.id === "AJOClblEstado") {
        const estado = (mutation.target.textContent || "").trim();
        if (!estado) return;

        $("#AJOCbuttons-loading").addClass("d-none");
        const estadoLower = estado.toLowerCase();
        if (estadoLower === "pendiente" || estadoLower === "vº bº 1" || estadoLower === "pe") {
          $("#AJOCbtn-aprobar").removeClass("d-none");
        } else {
          $("#AJOCbtn-aprobar").addClass("d-none");
        }
        console.log(' -1 AJOCmodalApproveOrders show.bs.modal');
      }
    });
  });
  // 4) Evento al abrir modal
  $(document).on("show.bs.modal", "#AJOCmodalApproveOrders", function () {
    $("#AJOCbuttons-loading").removeClass("d-none");
    $("#AJOCbtn-aprobar").addClass("d-none");

    const elementoEstado = document.getElementById("AJOClblEstado");
    if (elementoEstado) {
 ...
 $(document).on("hide.bs.modal", "#AJOCmodalApproveOrders", function () {
  observerEstado.disconnect();
   ...
   AJOCconfigurarEventos();
   AJOCconfigurarSeleccionFila();

   // Carga inicial de tabla (usa filtro por área por defecto)
   //PCPSOCOCVAJOCfiltrarPorArea();
   AJOCextraerDatosUsuario();
};