...
function actualizarOrden(accion, idservicio) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      const url = "http://192.168.0.5:8081/api/crtReqinterno?dbName=DONLUIS";
  ...
  function procesarSiguiente(index) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        // URL del servicio que quieres consumir
        const url = "http://192.168.0.5:8081/api/crtReqinterno";
        // Realizar la solicitud POST
        fetch(url, requestOptions)
          .then((response) => response.json()) // Puedes cambiar a .text() si la respuesta no es JSON
          .then((data1) => {
...
//   console.log("🚀 Inicializando fitosanidad con aprobación masiva");
//   returnApproveOrdersDoc();
//   selectRowLot();

//   // Cargar tabla automáticamente al inicializar
//   returnApproveOrdersFilter();
// });
