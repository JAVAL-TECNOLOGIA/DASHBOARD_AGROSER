$(document).ready(function () {
  AJSiniciarTablaReqInternos();
  $("#AJSmodalDetallesRequerimiento").on("hidden.bs.modal", function () {
    AJSlimpiarTablaStock();
  });
});
...
//================================================================================================================
function AJSiniciarTablaReqInternos() {
  const tabla = new DataTable("#AJStablaReqInternos", {
    responsive: true,
    autoWidth: false,
    pageLength: 10,
    language: {
    ...
    ajax: {
       url: "/almacen/api/req_interno_ajs/",
       type: "GET",
  ...
  $("#AJStablaReqInternos tbody").on("click", "tr", function () {
    const data = tabla.row(this).data();

    // Abrir el modal de detalles
    AJSabrirModalDetalles(data);
  });
}
...
function AJSabrirModalDetalles(Requerimiento) {
  if ($.fn.DataTable.isDataTable("#AJStablaDetalleProductos")) {
    $("#AJStablaDetalleProductos").DataTable().destroy();
  }

  // resetear el modal
  $("#AJSmodalDetallesRequerimiento").modal("hide");

  // mostrar el modal
  $("#AJSmodalDetallesRequerimiento").modal("show");

  //TITULO DEL MODAL
  $("#AJSreq-numero-documento").text(
    Requerimiento.documento || "no hay documento"
  );
  $("#AJSreq-fecha-documento").text(Requerimiento.fecha || "no hay fecha");
  $("#AJSreq-estado-documento").text(Requerimiento.estado || "no hay estado");

  // Cambiar color del contenedor del estado según el valor
  const estadoContainer = $("#AJSreq-estado-documento").parent();
  ...
  $("#AJSreq-periodo").val(Requerimiento.PERIODO || "no hay periodo");
  $("#AJSreq-punto-emision-codigo").val(
    Requerimiento.IDEMISOR || "no hay punto de emisión"
  );
  $("#AJSreq-punto-emision-nombre").val(
    Requerimiento.PUNTO_EMISION || "no hay punto de emisión"
  );
  $("#AJSreq-sucursal-nombre").val(Requerimiento.sucursal || "no hay sucursal");
  $("#AJSreq-almacen-nombre").val(Requerimiento.almacen || "no hay almacen");
  $("#AJSreq-documento").val(Requerimiento.documento || "no hay documento");
  $("#AJSreq-responsable-nombre").val(
    Requerimiento.responsable || "no hay responsable"
  );
  $("#AJSreq-area-nombre").val(Requerimiento.area || "no hay area");
  $("#AJSreq-observaciones").val(Requerimiento.nota_uso || "no hay observaciones");
  ...
  const tablaDetalles = new DataTable("#AJStablaDetalleProductos", {
    destroy: true,
    responsive: true,
    autoWidth: false,
    pageLength: 10,
    ...
    ajax: {
      url: `/almacen/api/req_internoDetalle_ajs/${idReqInterno}/`,
      type: "GET",
      dataSrc: function (json) {
        loading = false;
        ...
        if (
          ...
          setTimeout(() => {
            if (!$("#AJSmensaje-sin-datos").length) {
              $("#AJStablaDetalleProductos_wrapper").append(
                '<div id="AJSmensaje-sin-datos" class="alert alert-info mt-3">No hay detalles disponibles para este requerimiento</div>'
              );
            }
          }, 500);
    ...
    rowCallback: function (row, data) {
      $(row).on("click", function () {
        ...
        AJScargarTablaStock(idProducto);
      });
    },
  });
}
...
//================================================================================================================
function AJScargarTablaStock(idProducto) {
  // Limpiar tabla anterior si existe
  if ($.fn.DataTable.isDataTable("#AJStablaStockProductos")) {
    $("#AJStablaStockProductos").DataTable().destroy();
  }

  // Limpiar contenido pero mantener estructura
  $("#AJStablaStockProductos tbody").empty();

  console.log("Cargando stock para producto:", idProducto);

  const tablaStock = new DataTable("#AJStablaStockProductos", {
    destroy: true,
    responsive: true,
    autoWidth: false,
    pageLength: 1,
    ...
    ajax: {
      url: `/almacen/api/stock_producto_ajs/${idProducto}/`,
      type: "GET",
      dataSrc: function (json) {
...
function AJSlimpiarTablaStock() {
  ...
  $("#AJStablaStockProductos tbody").html(`
    <tr>
