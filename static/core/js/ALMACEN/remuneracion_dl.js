if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  $(document).ready(function () {
    ...
    $("#sueldosModalpresupuesto").on("shown.bs.modal", function () {
      if (
        !$.fn.DataTable.isDataTable("#sueldosTable") ||
        $("#sueldosTable").DataTable().data().length === 0
      ) {
    ...
    // Cuando se abre el modal de salarios
    $("#salariosModalpresupuesto").on("shown.bs.modal", function () {
      if (
        !$.fn.DataTable.isDataTable("#salariosTable") ||
        $("#salariosTable").DataTable().data().length === 0
      ) {
        if ($.fn.DataTable.isDataTable("#salariosTable")) {
          $("#salariosTable").DataTable().destroy();
        }
  ...
  function actualizarTotalSueldos() {
    $.ajax({
      url: "/almacen/almacen_sueldo_mensual/",
      type: "GET",
      dataType: "json",
  ...
  function actualizarTotalSalarios() {
    $.ajax({
      url: "/almacen/almacen_salario_mensual/",
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
  ...
  function inicializarAutocompleteDNI_Salario() {
    $("#dni_salario")
      ...
      .autocomplete({
        ...
        minLength: 8,
        select: function (event, ui) {
          if (!esDniGenericoSalario($("#dni_salario").val())) {
            let dniExistente = false;
  ...
  function initPresupuestoSueldos() {
    ...
    if ($.fn.DataTable.isDataTable("#sueldosTable")) {
      ...
      $("#sueldosTable").DataTable().ajax.reload();
      return;
    }
    ...
    obtenerTasaCargaSocial().then(function () {
      table = $("#sueldosTable").DataTable({
        ...
        ajax: {
          url: "/almacen/almacen_sueldos_dl/",
          dataSrc: "data",
        },
        columns: [
    ...
    $("#formSueldoSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/almacen/almacen_sueldos_dl/${formId}/`
            : "/almacen/almacen_sueldos_dl/",
  ...
  function initPresupuestoSalarios() {
    ...
    obtenerTasaCargaSocial_salarios().then(function () {
      tableSalarios = $("#salariosTable").DataTable({
        ...
        ajax: {
          url: "/almacen/almacen_salarios_dl/",
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/almacen/almacen_salarios_dl/${formId}/`
            : "/almacen/almacen_salarios_dl/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
  ...
  function eliminarSueldo(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/almacen/almacen_sueldos_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
  ...
  function eliminarSalario(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/almacen/almacen_salarios_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
              Swal.fire("¡Eliminado!", response.message, "success");
