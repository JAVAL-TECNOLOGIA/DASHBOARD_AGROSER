          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerAlmacen.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/almacen/objetivos_evaluacion/?campania="+ periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/almacen/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/almacen/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerAlmacen = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/almacen/objetivos_evaluacion/",
      detallesObjetivos: "/almacen/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/ALMACEN/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - ALMACEN",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de ALMACEN?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - ALMACEN",
        text: "Generando PDF desde ALMACEN y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en ALMACEN",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - ALMACEN",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar el módulo de gestión de correos electrónicos para ALMACEN
  EmailManagerAlmacen.inicializar();
