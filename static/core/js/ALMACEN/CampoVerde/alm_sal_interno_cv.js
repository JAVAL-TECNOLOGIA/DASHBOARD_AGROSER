...
$(document).ready(function () {
  ...
  window.tablaSalidasInternas = iniciarTablaSalInternosDL();
  configurarFiltrosSalidas();

  // MODAL PARA BUSCAR REQUERIMIENTO
  $("#btn-abrir-buscador-req").on("click", function () {
    abrirModalBuscarRequerimiento();
  });

  // Botón seleccionar requerimiento
  $("#btn-seleccionar-requerimiento").on("click", function () {
    const productosSeleccionadosActuales = obtenerProductosSeleccionados();
  ...
  $("#modalBuscarRequerimiento").on("hidden.bs.modal", function () {
    limpiarModalDocumentosReferencia();
    documentoSeleccionadoActual = null;
  });

  // Botón cerrar modal
  $("#btn-cerrar-modal-salida").on("click", function () {
    $("#modalDetallesSalidaInterna").modal("hide");
  });

  // Limpiar modal al cerrarse
  $("#modalDetallesSalidaInterna").on("hidden.bs.modal", function () {
    // Limpiar todos los campos
    $("#salida-id-documento").text("-");
    $("#salida-numero-documento").text("-");
    $("#salida-fecha-documento").text("-");
    $("#salida-estado-documento").text("-");
    $("#salida-empresa").val("");
    $("#salida-sucursal").val("");
    $("#salida-almacen-origen").val("");
    $("#salida-almacen-destino").val("");
    $("#salida-responsable").val("");
    $("#salida-motivo").val("");
    $("#salida-proyecto").val("");
    $("#salida-observaciones").val("");

    // Limpiar tabla de productos
    $("#tbody-productos-salida").html(`
      <tr>
  ...
  $("#filtro-numero-doc").on("keypress", function (e) {
    if (e.which === 13) {
      // Enter key
  ...
  $("#filtro-estado-doc, #filtro-fecha-desde, #filtro-fecha-hasta").on(
    "change",
    function () {
  ...
  $(document).on("change", ".chk-documento", function () {
    manejarSeleccionDocumento(this);
  });

  // Manejar selección de todos los documentos
  $("#chk-seleccionar-todos").on("change", function () {
    manejarSeleccionTodos(this);
  });
  ...
  inicializarBusquedaResponsables();
  ...
  $(document).on("click", ".btn-procesar-salida", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // Procesar documento
        procesarContabilizacionYCentralizacion(
          idIngresosAlidaAlm,
          datosDocumento
        );
      }
  ...
  $(document).on("change", "#encab-fecha", function () {
    ...
    if (fecha) {
      obtenerTipoCambioPorFecha(fecha);
    } else {
      $("#encab-tcambio").val("");
    }
  });
  ...
  $(document).on("shown.bs.modal", "#modalNuevaSalida", function () {
    ...
    if (fechaActual) {
      ...
      obtenerTipoCambioPorFecha(fechaActual);
    }
  });
  ...
  $("#btn-imprimir").on("click", function () {
    ...
    CrearPdfSalidaInterna(id_salida);
  });
...
function iniciarTablaSalInternosDL() {
  const tabla = new DataTable("#tablaSalidasInternas", {
    responsive: true,
    autoWidth: false,
    pageLength: 25,
    language: {
    ...
    ajax: {
      ...
      data: function (d) {
        ...
        // Filtros de fecha
        const fechaDesde = $("#desde_salidas").val();
        const fechaHasta = $("#hasta_salidas").val();
    ...
    rowCallback: function (row, data, index) {
      ...
      $(row)
        ...
        .on("click", function (e) {
          ...
          abrirModalDetallesSalida(idIngreso);
        });
...
function configurarFiltrosSalidas() {
  ...
  $("#btn-limpiar-salidas")
    ...
    .on("click", function () {
      ...
      $("#area_salidas").val("todos");
      $("#tipo_movimiento").val("todos");
      $("#desde_salidas").val("");
      $("#hasta_salidas").val("");
  ...
  $("#estado_salidas, #tipo_movimiento")
    .off("change")
    .on("change", function () {
...
// =====================================================
function abrirModalDetallesSalida(idIngresosAlidaAlm) {
  console.log("Abriendo modal de detalles para:", idIngresosAlidaAlm);

  // Mostrar el modal
  $("#modalDetallesSalidaInterna").modal("show");

  // Mostrar el spinner de carga
  $("#loading-salida-detalle").show();
  $("#contenido-salida-detalle").hide();

  // Actualizar el subtítulo del modal
  $("#modal-subtitulo").text(
    `Cargando información para: ${idIngresosAlidaAlm}`
  );

  // Hacer llamada AJAX para obtener los detalles
  $.ajax({
    url: `/almacen/salida-interna-cv/${idIngresosAlidaAlm}/`,
    type: "GET",
    dataType: "json",
    success: function (response) {
      ...
      if (response.success) {
        ...
        llenarEncabezadoSalida(response.encabezado);

        // Llenar tabla de productos
        llenarTablaProductos(response.detalles);

        // Llenar totales
        llenarTotales(response.totales, response.total_items);

        // Ocultar spinner y mostrar contenido
        $("#loading-salida-detalle").hide();
        $("#contenido-salida-detalle").show();
        ...
        $("#modal-subtitulo").text(`Documento: ${serie}-${numero}`);
        ...
        $("#btn-imprimir").data("id", idIngresosAlidaAlm);
        $("#btn-imprimir").show();
        

      } else {
        mostrarErrorEnModal(response.message || "Error al cargar los detalles");
      }
    },
    error: function (xhr, status, error) {
      ...
      mostrarErrorEnModal(errorMessage);
    },
  });
}
...
// =====================================================
function llenarEncabezadoSalida(encabezado) {
  // ID del documento
  $("#salida-id-documento").text(encabezado.IDINGRESOSALIDAALM || "-");
  ...
  if (encabezado.FECHA) {
    ...
    $("#salida-fecha-documento").text(fecha.toLocaleDateString("es-ES"));
  } else {
    $("#salida-fecha-documento").text("-");
  }
  ...
  $("#salida-estado-documento").text(estado.texto);
  ...
  // Empresa y sucursal
  $("#salida-empresa").val(encabezado.IDEMPRESA || "");
  $("#salida-sucursal").val(encabezado.IDSUCURSAL || "");

  // Almacenes
  $("#salida-almacen-origen").val(encabezado.IDALMACEN || "");
  $("#salida-almacen-destino").val(encabezado.IDALMACEND || "");

  // Responsable
  $("#salida-responsable").val(encabezado.IDRESPONSABLE || "");
  ...
  $("#salida-motivo").val(motivoTexto);

  // Proyecto
  $("#salida-proyecto").val(encabezado.IDPROYECTO || "");

  // Observaciones
  $("#salida-observaciones").val(encabezado.GLOSA || "");
}

// =====================================================
// FUNCIÓN PARA LLENAR LA TABLA DE PRODUCTOS
// =====================================================
function llenarTablaProductos(detalles) {
  const tbody = $("#tbody-productos-salida");
  tbody.empty();
  ...
  detalles.forEach(function (detalle) {
    const fila = `
      <tr>
        ...
        <td class="text-right font-weight-bold">${formatearNumero(
          detalle.CANTIDAD
        )}</td>
        <td class="text-right">${formatearNumero(detalle.PESO)}</td>
        <td class="text-right">${formatearMoneda(detalle.PRECIO)}</td>
        <td class="text-right font-weight-bold">${formatearMoneda(
          detalle.IMPORTE
        )}</td>
        <td>
...
// =====================================================
function llenarTotales(totales, totalItems) {
  // Total de productos
  $("#total-productos").text(totalItems || 0);

  if (totales) {
    // Total peso
    const totalPeso = totales.TotalPeso || 0;
    $("#total-peso").text(`${formatearNumero(totalPeso)} kg`);

    // Total importe
    const totalImporte = totales.TotalImporte || 0;
    $("#total-importe").text(formatearMoneda(totalImporte));
  } else {
    $("#total-peso").text("0.00 kg");
    $("#total-importe").text("S/ 0.00");
  }
}
...
function mostrarErrorEnModal(mensaje) {
  ...
  $("#contenido-salida-detalle").html(`
    <div class="text-center py-5">
      <i class="fas fa-exclamation-triangle fa-3x text-warning mb-3"></i>
      <h5 class="text-muted">Error al cargar los detalles</h5>
      <p class="text-muted">${mensaje}</p>
      <button type="button" class="btn btn-outline-primary" onclick="$('#modalDetallesSalidaInterna').modal('hide');">
        <i class="fas fa-times mr-1"></i>Cerrar
      </button>
    </div>
  `);

  $("#modal-subtitulo").text("Error en la carga");
}
...
function abrirModalBuscarRequerimiento() {
  ...
  // Mostrar el modal
  $("#modalBuscarRequerimiento").modal("show");
...
function limpiarModalDocumentosReferencia() {
  // Limpiar filtros
  $("#filtro-numero-doc").val("");
  $("#filtro-estado-doc").val("");
  $("#filtro-fecha-desde").val("");
  $("#filtro-fecha-hasta").val("");

  // Limpiar tabla
  $("#tabla-documentos-referencia tbody").empty();

  // Desmarcar checkbox principal
  $("#chk-seleccionar-todos").prop("checked", false);
}
...
function mostrarRequerimientos() {
  ...
  mostrarCargandoTablaDocumentos();
  ...
  const url = construirUrlConFiltros(
    "/almacen/api/buscar_requerimiento-cv/",
    filtros
  );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status === "success") {
        llenarTablaDocumentosReferencia(response.data);
      } else {
        mostrarErrorTablaDocumentos(response.message);
      }
    },
    error: function (xhr, status, error) {
      console.error("❌ Error en petición AJAX:", error);
      mostrarErrorTablaDocumentos("Error al cargar documentos de referencia");
    },
  });
}
...
function obtenerFiltrosActuales() {
  return {
    numero: $("#filtro-numero-doc").val().trim(),
    estado: $("#filtro-estado-doc").val().trim(),
    fecha_desde: $("#filtro-fecha-desde").val().trim(),
    fecha_hasta: $("#filtro-fecha-hasta").val().trim(),
    limit: 50,
  };
}
...
function mostrarCargandoTablaDocumentos() {
  const tbody = $("#tabla-documentos-referencia tbody");
  tbody.html(`
      <tr>
...
function mostrarErrorTablaDocumentos(mensaje) {
  const tbody = $("#tabla-documentos-referencia tbody");
  tbody.html(`
      <tr>
        <td colspan="8" class="text-center py-3 text-danger">
          ...
          <span>${mensaje}</span>
          <br>
          <button class="btn btn-sm btn-outline-primary mt-2" onclick="mostrarRequerimientos()">
...
function llenarTablaDocumentosReferencia(documentos) {
  const tbody = $("#tabla-documentos-referencia tbody");
  tbody.empty();
    ...
    const row = `
        <tr data-id="${doc.IDREQINTERNO}" style="font-size: 0.75rem;">
          ...
          <td style="padding: 0.15rem;" title="${
            doc.RAZON_SOCIAL || ""
          }">${truncarTexto(doc.RAZON_SOCIAL || "", 30)}</td>
...
function guardarDatosRequerimientoSeleccionado(checkbox) {
  ...
  obtenerDatosCompletosRequerimiento(idReqInterno)
    .then((datosCompletos) => {
    ...
    .catch((error) => {
      ...
      guardarDatosBasicosDesdeFilaTabla(checkbox);
    });
}

// Función para obtener datos completos del requerimiento desde la API
async function obtenerDatosCompletosRequerimiento(idReqInterno) {
  try {
    ...
    const url = `/almacen/api/buscar_requerimiento-cv/?idreqinterno=${idReqInterno}&limit=1`;
    ...
    console.error("❌ Error en obtenerDatosCompletosRequerimiento:", error);
    throw error;
  }
}

// Función fallback para obtener datos básicos desde la fila de la tabla
function guardarDatosBasicosDesdeFilaTabla(checkbox) {
...
function rellenarDocumentoReferencia(datos) {
  

  // Guardar IDREQINTERNO en campo oculto (múltiples formatos para compatibilidad)
  $("#doc-ref-idreqinterno").val(datos.IDREQINTERNO || datos.id || "");

  // Rellenar campos visibles usando tanto el formato antiguo como nuevo
  $("#doc-ref-doc").val(datos.TD || datos.td || "REQ");
  $("#doc-ref-serie").val(datos.SERIE || datos.serie || "");
  $("#doc-ref-numero").val(datos.NUMERO || datos.numero || "");
  $("#doc-ref-fecha").val(datos.fecha_iso || "");
  $("#doc-ref-responsable").val(datos.RAZON_SOCIAL || datos.razon_social || "");
  ...
  if (datos.IDRESPONSABLE || datos.idresponsable) {
    ...
    autocompletarResponsablePrincipal(idResponsableDocRef, nombreResponsableDocRef);
  }
  ...
  if (datos.OBSERVACION || datos.observacion) {
    ...
    $("#encab-glosa").val(observacionDocRef);
    
  }

  // Actualizar badge de estado con colores
  const estadoBadge = $("#doc-ref-estado");
...
function autocompletarResponsablePrincipal(idResponsable, nombreResponsable) {
 

  const selectResponsable = $("#encab-idresponsable");
  const inputDescripcion = $("#encab-desc-responsable");
...
function verificarDatosRequerimientoGuardados() {
...
function manejarSeleccionDocumento(checkbox) {
  ...
  if (isChecked) {
    ...
    guardarDatosRequerimientoSeleccionado(checkbox);

    // Cargar detalles del requerimiento seleccionado
    cargarDetallesRequerimiento(idReqInterno);
...
function actualizarCheckboxPrincipal() {
  ...
  const checkboxPrincipal = $("#chk-seleccionar-todos");
...
function cargarDetallesRequerimiento(idReqInterno) {
  ...
  mostrarCargandoDetallesProducto();

  // Construir URL de la API
  const url = `/almacen/api/detalle-requerimiento-cv/${idReqInterno}/`;
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (response.status === "success") {
        ...
        llenarTablaDetallesProducto(response.data, response.estadisticas);

        // Mostrar estadísticas si las hay
        if (response.estadisticas) {
          mostrarEstadisticasRequerimiento(response.estadisticas);
        }
      } else {
        mostrarErrorDetallesProducto(
    ...
    error: function (xhr, status, error) {
      ...
      mostrarErrorDetallesProducto(mensajeError);
    },
  });
}

// Función para mostrar indicador de carga en detalles
function mostrarCargandoDetallesProducto() {
  const tbody = $("#tabla-detalles-producto tbody");
  tbody.html(`
    <tr>
...
function mostrarErrorDetallesProducto(mensaje) {
  const tbody = $("#tabla-detalles-producto tbody");
  tbody.html(`
    <tr>
      <td colspan="13" class="text-center py-4 text-danger">
        ...
        <span>${mensaje}</span>
        <br>
        <button class="btn btn-sm btn-outline-primary mt-2" onclick="recargarDetallesActual()">
          <i class="fas fa-redo mr-1"></i>Reintentar
        </button>
      </td>
    </tr>
...
function recargarDetallesActual() {
  if (documentoSeleccionadoActual && documentoSeleccionadoActual.id) {
    cargarDetallesRequerimiento(documentoSeleccionadoActual.id);
  }
}

// Función para limpiar la tabla de detalles
function limpiarTablaDetallesProducto() {
  const tbody = $("#tabla-detalles-producto tbody");
  tbody.html(`
    <tr>
...
function llenarTablaDetallesProducto(productos, estadisticas) {
  const tbody = $("#tabla-detalles-producto tbody");
  tbody.empty();
  ...
  productos.forEach((producto, index) => {
    ...
    const row = `
      <tr style="font-size: 0.75rem;" data-item="${producto.ITEM}">
        ...
        }">${truncarTexto(producto.DESCRIPCION || "", 40)}</td>
        <td style="padding: 0.25rem; text-align: center;">${
          producto.IDMEDIDA || ""
        }</td>
...
function mostrarEstadisticasRequerimiento(estadisticas) {
...
function manejarSeleccionProducto(checkbox) {
  ...
  if (checkbox.checked) {
    ...
    if (productoCompleto) {
      ...
      const producto = obtenerDatosProductoDeFila(fila, item);
  ...
  actualizarCheckboxPrincipalProductos();

  
}

// Función para manejar selección/deselección de todos los productos
function manejarSeleccionTodosProductos(checkbox) {
...
// Función para actualizar el estado del checkbox principal
function actualizarCheckboxPrincipalProductos() {
  const checkboxPrincipal = document.getElementById("selectAllProductos");
  const checkboxesProductos = document.querySelectorAll(".chk-producto");
...
function obtenerDatosProductoDeFila(fila, item) {
...
function limpiarSeleccionProductos() {
  ...
  const checkboxes = document.querySelectorAll(
    ".chk-producto, #selectAllProductos"
  );
...
function transferirProductosAModalSalida(productos, requerimiento) {
  

  // Actualizar información del documento de referencia
  actualizarDocumentoReferencia(requerimiento);

  // Llenar la tabla de productos seleccionados
  llenarTablaProductosSeleccionados(productos);
...
function actualizarDocumentoReferencia(requerimiento) {
  $("#doc-ref-doc").val(requerimiento.td || "REQ");
  $("#doc-ref-serie").val(requerimiento.serie || "");
  $("#doc-ref-numero").val(requerimiento.numero || "");
  $("#doc-ref-fecha").val(requerimiento.fecha_iso || "");
  $("#doc-ref-responsable").val(requerimiento.razon_social || "");
...
function llenarTablaProductosSeleccionados(productos) {
  ...
  if (!productos || productos.length === 0) {
    ...
    `);
    $("#total-productos-seleccionados").text("0");
    return;
  }
    ...
    const row = `
      <tr style="font-size: 0.8rem;" data-item="${producto.ITEM}">
        ...
        <td style="padding: 0.4rem;" title="${
          producto.DESCRIPCION
        }">${truncarTexto(producto.DESCRIPCION, 50)}</td>
        ...
        <td style="padding: 0.4rem; text-align: center;">
          <button 
            ...
            onclick="eliminarProductoSeleccionado('${producto.ITEM}')"
  ...
  // Actualizar contador
  $("#total-productos-seleccionados").text(productos.length);

  
}

// Función para eliminar un producto de la tabla de productos seleccionados
function eliminarProductoSeleccionado(item) {
  ...
  $("#total-productos-seleccionados").text(filasRestantes);
...
function obtenerProductosParaSalida() {
  const productos = [];
...
function limpiarTablaProductosSeleccionados() {
  $("#tabla-productos-seleccionados tbody").html(`
    <tr>
      <td colspan="8" class="text-center py-3 text-muted">
        <i class="fas fa-info-circle mr-2"></i>
  ...
  $("#doc-ref-idreqinterno").val("");
  $("#doc-ref-doc").val("");
  $("#doc-ref-serie").val("");
  $("#doc-ref-numero").val("");
  $("#doc-ref-fecha").val("");
  $("#doc-ref-responsable").val("");
  $("#doc-ref-estado")
...
function recopilarDatosEncabezado() {
  ...
  const encabezado = {
    ...
    IDRESPONSABLE: $("#encab-idresponsable").val() || "",
    GLOSA: $("#encab-glosa").val() || "",
    IDMONEDA: $("#encab-idmoneda").val() || "01",
    TCAMBIO: parseFloat($("#encab-tcambio").val()) || 3.759,
...
function recopilarDatosProductosCompletos() {
...
function recopilarDatosDocumentoReferencia() {
...
//================================================================================================================
function recopilarProductosParaGuardado() {
  ...
  filas.forEach((fila, index) => {
    try {
      ...
      const idReqInterno = $("#doc-ref-idreqinterno").val() || "";
...
//================================================================================================================
async function probarNuevaImplementacionAPI() {
  ...
  try {
    ...
    const datosCompletos = await obtenerDatosCompletosRequerimiento(
      idReqInternoEjemplo
    );

    
    rellenarDocumentoReferencia(datosCompletos);
    ...
    if (window.datosRequerimientoCompletos) {
      ...
      const documentoRef = recopilarDatosDocumentoReferencia();
...
// probarNuevaImplementacionAPI()

//================================================================================================================
// FUNCIÓN DE PRUEBA COMPLETA DEL FLUJO DE GUARDADO
//================================================================================================================
async function probarFlujoCompletoGuardado() {
  ...
  try {
    ...
    const estructura = generarEstructuraFinalParaGuardar();
...
// 1. probarNuevaImplementacionAPI() - Solo API
// 2. probarFlujoCompletoGuardado() - Flujo completo
// 3. probarProcesamientoReqInterno() - Solo procesamiento REQINTERNO

//================================================================================================================
// FUNCIÓN DE PRUEBA PARA EL PROCESAMIENTO REQINTERNO
//================================================================================================================
async function probarProcesamientoReqInterno() {
  ...
  try {
    ...
    if (productosExtraidos.length > 0) {
      ...
      console.log(
        "💡 Para ejecutar el procesamiento real, usa: procesarProductosReqInterno(datosDocumento, responseProceso)"
      );
    } else {
...
function verificarEstadoParaReqInterno() {
  ...
  const idFromForm = $("#doc-ref-idreqinterno").val();
  
}

// Para ejecutar las pruebas desde la consola:
// 1. verificarEstadoParaReqInterno() - Verificar estado actual
// 2. probarProcesamientoReqInterno() - Probar extracción de datos
// 3. procesarProductosReqInterno(datosDocumento, responseProceso) - Ejecutar procesamiento real

//================================================================================================================
// FUNCIÓN DE DIAGNÓSTICO COMPLETO PARA PROBLEMAS DE GUARDADO
//================================================================================================================
function diagnosticarProblemaGuardado() {
  ...
  const modalSalida = document.getElementById("modalNuevaSalida");
  const tablaProductos = document.querySelector(
    "#tabla-productos-seleccionados tbody"
  );
  ...
  fetch("/almacen/salida-interna/", { method: "GET" })
    .then((response) => {
      console.log(
        "   • Conectividad API:",
        response.status,
...
//================================================================================================================
function generarEstructuraFinalParaGuardar() {
  

  try {
    // Recopilar todos los datos
    const encabezado = recopilarDatosEncabezado();
    const productos = recopilarProductosParaGuardado();
    const documentoReferencia = recopilarDatosDocumentoReferencia();
...
function guardarSalidaInterna() {
  ...
  try {
    ...
    const datosEnvio = {
      encabezado: recopilarDatosEncabezado(),
      productos: recopilarProductosParaGuardado(),
      documento_referencia: recopilarDatosDocumentoReferencia(),
    };
...
function recuperarDatosOriginalesParaReqInterno(
  datosDocumento,
  responseProceso
) {
  ...
  try {
    ...
    const encabezadoBase = recopilarDatosEncabezado();
    const productosBase = recopilarProductosParaGuardado();
    const documentoReferenciaBase = recopilarDatosDocumentoReferencia();
    ...
    enviarEstructuraAApiReqInterno(
      estructuraCompleta,
      datosDocumento,
      responseProceso
    );
...
function enviarEstructuraAApiReqInterno(
  estructuraCompleta,
  datosDocumento,
  responseProceso
) {
  ...
  const endpoint = "/almacen/api/procesar-requerimiento-cv/";
...
function procesarProductosReqInterno(datosDocumento, responseProceso) {
 
  try {
    // ✅ RECUPERAR DATOS Y ENVIAR AUTOMÁTICAMENTE A LA API
    console.log("🚀 ===== INICIANDO PROCESAMIENTO REQINTERNO =====");

    const datosRecuperados = recuperarDatosOriginalesParaReqInterno(
      datosDocumento,
      responseProceso
    );
    ...
    // La función recuperarDatosOriginalesParaReqInterno ya se encarga de enviar los datos
    // No necesitamos hacer nada más aquí, todo el flujo está automatizado
  } catch (error) {
    console.error("❌ Error en procesarProductosReqInterno:", error);
    mostrarErrorReqInterno(
      datosDocumento,
      responseProceso,
...
function obtenerIdReferenciaRequerimientoMejorado() {
...
function diagnosticarEstadoParaReqInterno() {
  ...
  diagnostico.detalles.datosRequerimiento = {
    ...
    campoFormulario: $("#doc-ref-idreqinterno").val() || null,
  };

  const idreferencia = obtenerIdReferenciaRequerimientoMejorado();
  if (!idreferencia) {
...
function validarDatosParaAPI(productos) {
...
function obtenerIdReferenciaRequerimiento() {
...
function enviarProductosReqInternoAPI(
  productos,
  datosDocumento,
  responseProceso
) {
  ...
  const timeoutId = setTimeout(() => {
    ...
    mostrarTimeoutReqInterno(datosDocumento, responseProceso);
  ...
  $.ajax({
    ...
    success: function (response, textStatus, xhr) {
      ...
      if (response.status === "success") {
        ...
        mostrarExitoFinalCompleto(datosDocumento, responseProceso, response);
    ...
    error: function (xhr, textStatus, errorThrown) {
      ...
      mostrarErrorReqInterno(datosDocumento, responseProceso, mensajeFinal);
    },
...
window.debugReqInterno = function () {
  ...
  const debug = {
    ...
    elementos_dom: {
      campo_idreqinterno: {
        existe: $("#doc-ref-idreqinterno").length > 0,
        valor: $("#doc-ref-idreqinterno").val(),
        visible: $("#doc-ref-idreqinterno").is(":visible"),
      },
  ...
  if (typeof diagnosticarEstadoParaReqInterno === "function") {
    debug.diagnostico_automatico = diagnosticarEstadoParaReqInterno();
  }
...
window.probarFlujoPocesaQueRrequirimiento = function () {
  ...
  try {
    ...
    procesarProductosReqInterno(datosDocumentoPrueba, responseProcesoPrueba);
...
function mostrarExitoFinalCompleto(
  datosDocumento,
  responseProceso,
  responseReqInterno
) {
...
function mostrarExitoFinalSinReqInterno(datosDocumento, responseProceso) {
...
function mostrarErrorReqInterno(datosDocumento, responseProceso, mensajeError) {
...
function mostrarTimeoutReqInterno(datosDocumento, responseProceso) {
...
function procesarContabilizacionYCentralizacion(
  idIngresosAlidaAlm,
  datosDocumento
) {
  ...
  const timeoutId = setTimeout(() => {
    ...
    mostrarTimeoutProceso(datosDocumento);
  }, 30000); // 30 segundos timeout

  // Llamar al endpoint de contabilización y centralización
  $.ajax({
    url: "/almacen/salida-interna-procesar-cv/",
    type: "POST",
    data: JSON.stringify(datosEnvio),
    contentType: "application/json",
    headers: {
    ...
    success: function (response) {
      ...
      if (response.success) {
        // Proceso exitoso
        mostrarExitoCompleto(datosDocumento, response);
      } else {
        // Proceso falló
        mostrarErrorProceso(datosDocumento, response);
      }
    },
    error: function (xhr, status, error) {
      ...
      mostrarErrorProceso(datosDocumento, { error: mensajeError });
    },
  });
}

// Función para mostrar éxito completo (documento creado + proceso exitoso)
function mostrarExitoCompleto(datosDocumento, responseProceso) {
  ...
  if (
    ...
    procesarProductosReqInterno(datosDocumento, responseProceso);
...
function mostrarErrorProceso(datosDocumento, responseProceso) {
...
function mostrarExitoBasico(response) {
...
function mostrarTimeoutProceso(datosDocumento) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      mostrarOpcionesProcesamiento(datosDocumento);
    }
  });
}

// Función para mostrar opciones de procesamiento manual
function mostrarOpcionesProcesamiento(datosDocumento) {
  Swal.fire({
    icon: "info",
...
function inicializarBusquedaResponsables() {
  ...
  $("#encab-idresponsable").on("change", function () {
    const codigo = $(this).val();
    
    actualizarDescripcionResponsable(codigo);
    ...
    if (responsable) {
      $("#encab-desc-responsable").val(responsable.nombre);
    }
  });
...
function cargarResponsables() {
  ...
  $.ajax({
    url: "/almacen/api/responsables-cv/",
    type: "GET",
    dataType: "json",
    success: function (response) {
      ...
      if (response.success && response.data) {
        ...
        llenarSelectResponsables(response.data);
...
function llenarSelectResponsables(responsables) {
  const select = $("#encab-idresponsable");
...
function actualizarDescripcionResponsable(idResponsable) {
  const descripcionInput = $("#encab-desc-responsable");
  ...
  if (responsable) {
    ...
    buscarResponsablePorId(idResponsable);
  }
}

// Función para buscar un responsable específico por ID
function buscarResponsablePorId(idResponsable) {
  console.log(`🔍 Buscando responsable por ID: ${idResponsable}`);

  $.ajax({
    url: `/almacen/api/responsables-cv/?idresponsable=${idResponsable}`,
    type: "GET",
    dataType: "json",
    success: function (response) {
...
function agregarFuncionalidadBusqueda() {
  ...
  $(document).on("click", function (e) {
    if (
      !$(e.target).closest("#encab-desc-responsable, #sugerencias-responsables")
        .length
    ) {
      ocultarSugerencias();
    }
  ...
  selectResponsable.on("change", function () {
    ...
    if (idSeleccionado && idSeleccionado !== "") {
      actualizarDescripcionResponsable(idSeleccionado);
    } else {
      descripcionInput.val("");
    }
  });
...
function buscarYAutocompletarPorCodigo(codigo) {
  ...
  if (coincidencias.length === 1) {
    ...
    $("#encab-idresponsable").val(responsable.idresponsable);
    $("#encab-desc-responsable").val(responsable.nombre);
    console.log(
    ...
    mostrarSugerenciasCodigos(coincidencias, codigo);
    return;
  }

  // Si no encuentra localmente y el código tiene suficientes dígitos, buscar en API
  if (codigo.length >= 3) {
    buscarCodigoEnAPI(codigo);
  }
}

// Función para buscar código en la API
function buscarCodigoEnAPI(codigo) {
  const codigoFormateado = codigo.padStart(6, "0");

  $.ajax({
    url: `/almacen/api/responsables-cv/?idresponsable=${codigoFormateado}`,
    type: "GET",
    dataType: "json",
    success: function (response) {
      if (response.success && response.data && response.data.length > 0) {
...
function mostrarSugerenciasCodigos(responsables, codigoBuscado) {
  ...
  responsables.slice(0, 6).forEach((responsable) => {
    ...
    configurarEventosSugerencia(item);
...
function mostrarSugerenciasPorNombre(termino) {
  ...
  if (coincidencias.length > 0) {
    mostrarDropdownSugerencias(coincidencias, termino);
  } else {
    // Si no hay coincidencias locales, buscar en la API
    buscarEnAPIPorNombre(termino);
  }
}

// Función para buscar en la API por nombre
function buscarEnAPIPorNombre(termino) {
  $.ajax({
    url: `/almacen/api/responsables-cv/?nombre=${encodeURIComponent(termino)}`,
    type: "GET",
    dataType: "json",
    success: function (response) {
      if (response.success && response.data && response.data.length > 0) {
        mostrarDropdownSugerencias(response.data.slice(0, 8), termino);
        console.log(
          `✅ Encontrados ${response.data.length} responsables por API`
        );
      } else {
        mostrarSinSugerencias(termino);
      }
    },
    error: function (xhr, status, error) {
...
function mostrarDropdownSugerencias(responsables, terminoBuscado) {
  const contenedor = $("#sugerencias-responsables");
  contenedor.empty();

  responsables.forEach((responsable, index) => {
    // Resaltar el término buscado en el nombre
    const nombreResaltado = resaltarTexto(responsable.nombre, terminoBuscado);
...
function configurarEventosSugerencia(item) {
...
function mostrarSinSugerencias(termino) {
  const contenedor = $("#sugerencias-responsables");
  contenedor
    .html(
      `
...
function mostrarErrorCargaResponsables() {
...
window.verificarBackend = function () {
  ...
  return new Promise((resolve, reject) => {
    $.ajax({
      url: "/almacen/api/procesar-requerimiento-cv/",
      type: "POST",
...
window.probarRecuperacionDatos = function (enviarAApi = true) {
  ...
  try {
    if (enviarAApi) {
      ...
      const resultado = recuperarDatosOriginalesParaReqInterno(
      ...
      const enviarOriginal = window.enviarEstructuraAApiReqInterno;
      window.enviarEstructuraAApiReqInterno = function () {
      ...
      window.enviarEstructuraAApiReqInterno = enviarOriginal;
...
function obtenerTipoCambioPorFecha(fecha) {
  console.log("🔄 Función obtenerTipoCambioPorFecha ejecutándose con fecha:", fecha);
  
  // Mostrar loading en el campo
  $("#encab-tcambio").val("...").prop("disabled", true);

  $.ajax({
    url: "/almacen/api/tcambio-cv/?fecha=" + encodeURIComponent(fecha),
    method: "GET",
    dataType: "json",
    success: function (response) {
      ...
      if (response.success && response.data && response.data.length > 0) {
        ...
        $("#encab-tcambio").val(tc).prop("disabled", false);
        ...
        $("#encab-tcambio").val("").prop("disabled", false);
        if (typeof toastr !== "undefined") {
          toastr.warning(
...
function CrearPdfSalidaInterna(id_salida){
  ...
  // Construir URL de la API
  const apiUrl = `/almacen/api/salida-interna-cv/${id_salida}/`;
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      crearPrevisualizacionPDF(response.data);
    },
    error: function(xhr, status, error) {
...
/* 
function crearPrevisualizacionPDF(datos) {
...
function crearPrevisualizacionPDF(datos) {
  ...
  try {
    ...
    doc.text('AGROINDUSTRIA CAMPO VERDE S.A.C.', margin, yPos);
    yPos += 5;
    
    doc.setFontSize(10);
    doc.setFont('courier', 'normal');
    doc.text('CAL. CONTRALMIRANTE MONTERO NRO. 411 INT. 404 URB. CAMPO DE POLO', margin, yPos);
    yPos += 4;
    doc.text('RUC: 20502072006', margin, yPos);
