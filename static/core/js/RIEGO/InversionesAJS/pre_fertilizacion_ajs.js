...
function iniciarTablaResumen_SG(nombrePestaña) {
  ...
  if (nombrePestaña == "PLANTINES") {
    ...
    const tabla = $("#tablaFertilizacionPlantines_SG").DataTable({
      ...
      ajax: {
        url: "/riego/api/riego_presupuesto_fertilizacion_ajs/",
        type: "GET",
        data: function (d) {
          d.idfase = 1;
          d.idvariedad = 1;
...
function guardarNuevoPrograma_SG() {
  ...
  if (!hayErrores) {
    ...
    const url = esEdicion
      ? `/riego/api/riego_presupuesto_fertilizacion_ajs/${idregistro}/`
      : "/riego/api/riego_presupuesto_fertilizacion_ajs/";
...
function editarFertilizacion_SG(idRegistro) {
  ...
  $.ajax({
    url: `/riego/api/riego_presupuesto_fertilizacion_ajs/${idRegistro}/`,
    type: "GET",
    success: function (response) {
...
function cargarProductosPrograma_AC(idPrograma) {
  ...
  const tabla = $("#tablaDetalleProductos_AC").DataTable({
    ...
    ajax: {
      url: `/riego/api/riego_dprefertilizacion_ajs/`,
      type: "GET",
      dataSrc: function (json) {
        ...
        const datosFiltrados = json.data.filter(function (item) {
          // Convertir el IDRIEGO_PREFERTILIZACION a número también
          const idPreFert = parseInt(item.IDRIEGO_PREFERTILIZACION, 10);
    ...
    success: function (response) {
      ...
      if (response && response.data) {
        ...
        const productosAsociados = response.data.filter(function (item) {
          return (
            parseInt(item.IDRIEGO_PREFERTILIZACION, 10) ===
...
function guardarProductoAutumnCrisp() {
  ...
  const datos = {
    IDRIEGO_PREFERTILIZACION: programaId,
  ...
  let url = "/riego/api/riego_dprefertilizacion_ajs/";
  let tipo = "POST";

  if (idProducto) {
    url = `/riego/api/riego_dprefertilizacion_ajs/${idProducto}/`;
    tipo = "PUT";
  }
...
function guardarProductoSweetGlobe() {
  ...
  if (esEdicion) {
    url = `/riego/api/riego_dprefertilizacion_ajs/${productoId}/`;
    metodo = "PUT";
  } else {
  }
...
function editarProductoSweetGlobe(idProducto) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $("#programaId").val(producto.IDRIEGO_PREFERTILIZACION);
...
function cargarLotes(idVariedad, selectorId = null) {
  return new Promise((resolve, reject) => {
    $.ajax({
      ...
      success: function (response) {
        // Determinar qué selector usar basado en la variedad o el parámetro
        let selectLote;
        if (selectorId) {
          selectLote = $(selectorId);
        } else {
          // Autodetectar basado en la variedad
          if (idVariedad === 1) {
            selectLote = $("#selectLote"); // Sweet Globe
          } else if (idVariedad === 2) {
            selectLote = $("#selectLote_AC"); // Autumn Crisp
          } else if (idVariedad === 3) {
            selectLote = $("#selectLote_MC"); // Moscatel
          } else if (idVariedad === 4) {
            selectLote = $("#selectLote_SUGRA"); // Sugra
          } else {
            console.error(
              "Variedad no reconocida para cargar lotes:",
              idVariedad
            );
        ...
        if (
          ...
          response.data.forEach(function (lote) {
            const option = `<option value="${lote.ID}">
          ...
          selectLote.append(
            '<option value="" disabled>No hay lotes disponibles para esta variedad</option>'
          );
        }
