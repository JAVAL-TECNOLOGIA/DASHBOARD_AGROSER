...
function initServiciosTable() {
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/riego/riego_servicios_ajs/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/riego/riego_servicios_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/riego/riego_servicios_totals_ajs/?year=${yearVal}`,
    type: "GET",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/riego/riego_servicios_ajs/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/riego/riego_servicios_ajs/${id}/`
      : "/riego/riego_servicios_ajs/",
