...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/riego/riego_servicios_dl/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    ajax: {
      url: "/riego/riego_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
   ...
   */
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      // Cargar los totales desde la API
      $.ajax({
        url: "/riego/riego_servicios_totals_dl/",
        type: "GET",
        success: function (response) {
          const meses = [
            "Enero",
          ...
          meses.forEach((mes, index) => {
            const cantidadIndex = 6 + index * 2; // Ajustado el índice inicial a 8
            const precioIndex = cantidadIndex + 1;
            const cantidadKey = `Total_${mes}_Cantidad`;
            const precioKey = `Total_${mes}_Precio`;
            ...
            $(api.column(precioIndex).footer()).html(`
                        <div class="text-right">
                            <strong>S/. ${formatNumber(
                              response[precioKey] || 0,
                              2
                            )}</strong>
                        </div>
                    `);
          ...
          // Actualizar widget
          $("#stats_serviciosterceros").html(
            `S/. ${formatNumber(response.Total_General_Precio || 0, 2)}`
          );
        },
        error: function (xhr, status, error) {
          console.error("Error al cargar los totales:", error);
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/riego/riego_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/riego/riego_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/riego/riego_servicios_dl/${id}/`
      : "/riego/riego_servicios_dl/",
