...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/riego/riego_capex/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          // Recargar la tabla
          dt.ajax.url("/riego/riego_capex/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/riego/riego_capex/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/riego/riego_capex/" + formId + "/" : "/riego/riego_capex/",
