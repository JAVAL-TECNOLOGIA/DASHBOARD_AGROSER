/*-- =====================================================================================================================================================
-- Modulo	            : CONTABILIDAD - PRESUPUESTO SUMINISTROS
-- Autor			        : JHON GUTIERREZ
-- Fecha de creación  : 2024-10-18
-- Descripción		    : Controla la logica de los suministros para el área de CONTABILIDAD
-- Parámetros		      : 
-- =====================================================================================================================================================
-- Resumen de modificaciones:
--  2024-10-18: Creación inicial del módulo
--  2026-01-01: Se agregó filtro por campaña (year) en todas las tablas de suministros
--              - El filtro se obtiene del select #filtroAnioPresupuesto
--              - Se envía como parámetro 'year' al backend (ej: ?year=CAMP2026)
--              - El backend filtra por el campo ID_CAMPANIA en la tabla TIC_suministros
-- =====================================================================================================================================================
-- ENDPOINTS UTILIZADOS:
--  - /produccionpalta/produccionpalta_combustibles-lubricantes_ajs/     (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_utiles-oficina_ajs/               (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_equipos-computo_ajs/              (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_otros-suministros_ajs/            (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_material-construccion_ajs/        (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_repuestos-accesorios_ajs/         (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_equipos-uit_ajs/                  (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_materiales-agricultura_ajs/       (GET, POST, PUT, DELETE)
--  - /produccionpalta/produccionpalta_equipos-proteccion_ajs/           (GET, POST, PUT, DELETE)
-- =====================================================================================================================================================
-- FILTRO POR CAMPAÑA:
--  - El parámetro 'year' se envía en la URL de cada petición AJAX
--  - Formato del valor: 'CAMP' + año (ej: CAMP2026, CAMP2025, CAMP2024)
--  - Si no se especifica, el backend devuelve todos los registros del área
--  - El filtro se aplica automáticamente en la carga inicial de la página
-- =====================================================================================================================================================
-- PRESUPUESTO CONTABILIDAD - DON LUIS
-- =====================================================================================================================================================*/
...
// =====================================================================================
// FUNCIONES PARA CALCULAR TOTALES DE LAS CARDS DE SUMINISTROS
// =====================================================================================
// Estas funciones obtienen los totales desde el endpoint produccionpalta_suministros_totals_ajs
// y actualizan las cards de cada tipo de suministro.
// 
// IMPORTANTE: Se envía el parámetro 'year' para filtrar por campaña
// =====================================================================================
...
function calcularTotalPresupuesto() {
  ...
  return new Promise((resolve, reject) => {
    $.ajax({
      url: `/produccionpalta/produccionpalta_suministros_totals_ajs/?year=${yearVal}`,
      type: "GET",
      dataType: "json",
...
// =====================================================================================
// Descripción: Inicializa la tabla DataTable para Combustibles y Lubricantes
// Autor: JHON GUTIERREZ
// Modificado: Se agregó filtro por campaña en la carga inicial
// =====================================================================================
// IMPORTANTE: Esta función obtiene el valor del filtro de campaña (#filtroAnioPresupuesto)
// y lo envía como parámetro 'year' al endpoint del backend para filtrar los datos.
// Si el filtro no tiene valor, usa el año actual como valor por defecto.
// =====================================================================================
function initPresupuestoMateriales() {

    // ---------------------------------------------------------------------------------
    // OBTENER VALOR DEL FILTRO DE CAMPAÑA
    // ---------------------------------------------------------------------------------
    // Se obtiene el valor del select #filtroAnioPresupuesto
    // Si no existe o está vacío, se usa el año actual con formato 'CAMP' + año
    // Ejemplo: 'CAMP2026'
    // ---------------------------------------------------------------------------------
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  
  // Log para debugging - verificar qué valor se está usando
  console.log("[initPresupuestoMateriales] Filtro de campaña aplicado:", yearVal);
  
  // ---------------------------------------------------------------------------------
  // VERIFICAR SI LA TABLA YA EXISTE
  // ---------------------------------------------------------------------------------
  // Si la tabla ya fue inicializada, solo recargamos los datos con el nuevo filtro
  // Esto evita errores de reinicialización de DataTables
  // ---------------------------------------------------------------------------------
  if ($.fn.DataTable.isDataTable("#materialOficinaTable")) {
    $("#materialOficinaTable").DataTable().ajax.url("/produccionpalta/produccionpalta_combustibles-lubricantes_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  // ---------------------------------------------------------------------------------
  // Se crea la tabla con todas las configuraciones necesarias
  // El parámetro 'year' se envía en la URL del ajax para filtrar por campaña
  // ---------------------------------------------------------------------------------
  var table = $("#materialOficinaTable").DataTable({
    ...
    // ---------------------------------------------------------------------------------
    // AJAX - OBTENER DATOS DEL SERVIDOR
    // ---------------------------------------------------------------------------------
    // URL del endpoint con el parámetro 'year' para filtrar por campaña
    // El backend debe leer este parámetro y filtrar los registros por ID_CAMPANIA
    // ---------------------------------------------------------------------------------
    ajax: {
      url: "/produccionpalta/produccionpalta_combustibles-lubricantes_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
  ...
  $("#materialOficinaTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_material(id, "produccionpalta/produccionpalta_combustibles-lubricantes_ajs");
  });
  ...
  $("#registerMaterialForm").on("submit", function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_combustibles-lubricantes_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_combustibles-lubricantes_ajs/",
...
function initPresupuestoUtilesOficina() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#utilesOficinaTable")) {
    $("#utilesOficinaTable").DataTable().ajax.url("/produccionpalta/produccionpalta_utiles-oficina_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#utilesOficinaTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_utiles-oficina_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    columns: [
  ...
  $("#utilesOficinaTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_utiles(id, "produccionpalta/produccionpalta_utiles-oficina_ajs");
  });
  ...
  $("#registerMaterialForm_utiles").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_utiles-oficina_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_utiles-oficina_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposComputo() {
  ...
  if ($.fn.DataTable.isDataTable("#equipoComputoTable")) {
    $("#equipoComputoTable").DataTable().ajax.url("/produccionpalta/produccionpalta_equipos-computo_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#equipoComputoTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_equipos-computo_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#equipoComputoTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_computo(id, "produccionpalta/produccionpalta_equipos-computo_ajs");
  });
  ...
  $("#registerMaterialForm_computo").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_equipos-computo_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_equipos-computo_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoOtrosSuministros() {
  ...
  if ($.fn.DataTable.isDataTable("#otrosSuministrosTable")) {
    $("#otrosSuministrosTable").DataTable().ajax.url("/produccionpalta/produccionpalta_otros-suministros_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#otrosSuministrosTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_otros-suministros_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#otrosSuministrosTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_otros_suministros(id, "produccionpalta/produccionpalta_otros-suministros_ajs");
  });
  ...
  $("#registerMaterialForm_otros").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_otros-suministros_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_otros-suministros_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoMaterialConstruccion() {
  ...
  if ($.fn.DataTable.isDataTable("#materialConstruccionTable")) {
    $("#materialConstruccionTable").DataTable().ajax.url("/produccionpalta/produccionpalta_material-construccion_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#materialConstruccionTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_material-construccion_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#materialConstruccionTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_construccion(id, "produccionpalta/produccionpalta_material-construccion_ajs");
  });
  ...
  $("#registerMaterialForm_construccion").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_material-construccion_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_material-construccion_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoRepuestosAccesorios() {
  ...
  if ($.fn.DataTable.isDataTable("#repuestosAccesoriosTable")) {
    $("#repuestosAccesoriosTable").DataTable().ajax.url("/produccionpalta/produccionpalta_repuestos-accesorios_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#repuestosAccesoriosTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_repuestos-accesorios_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    columns: [
      { data: "id", title: "ID", visible: false },
  ...
  $("#repuestosAccesoriosTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_accesorios(id, "produccionpalta/produccionpalta_repuestos-accesorios_ajs");
  });
  ...
  $("#registerMaterialForm_repuestos").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_repuestos-accesorios_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_repuestos-accesorios_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposUIT() {
  ...
  if ($.fn.DataTable.isDataTable("#equiposuitTable")) {
    $("#equiposuitTable").DataTable().ajax.url("/produccionpalta/produccionpalta_equipos-uit_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#equiposuitTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_equipos-uit_ajs/?year=" +yearVal,
      dataSrc: "data",
    },
  ...
  $("#equiposuitTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_uit(id, "produccionpalta/produccionpalta_equipos-uit_ajs");
  });
  ...
  $("#registerMaterialForm_uit").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_equipos-uit_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_equipos-uit_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoMaterialAgricultura() {
  ...
  if ($.fn.DataTable.isDataTable("#materialagriculturaTable")) {
    $("#materialagriculturaTable").DataTable().ajax.url("/produccionpalta/produccionpalta_materiales-agricultura_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#materialagriculturaTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_materiales-agricultura_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    fixedColumns: {
      right: 1, // Fija la última columna (acciones) a la derecha
  ...
  $("#materialagriculturaTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_agricultura(id, "produccionpalta/produccionpalta_materiales-agricultura_ajs");
  });
  ...
  $("#registerMaterialForm_agricultura").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_materiales-agricultura_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_materiales-agricultura_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposProteccion() {
  ...
  if ($.fn.DataTable.isDataTable("#equipoProteccionTable")) {
    $("#equipoProteccionTable").DataTable().ajax.url("/produccionpalta/produccionpalta_equipos-proteccion_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#equipoProteccionTable").DataTable({
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_equipos-proteccion_ajs/?year=" + yearVal,
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#equipoProteccionTable tbody").on("click", ".delete-btn", function () {
    var id = $(this).data("id");
    eliminarProducto_proteccion(id, "produccionpalta/produccionpalta_equipos-proteccion_ajs");
  });
  ...
  $("#registerMaterialForm_proteccion").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/produccionpalta/produccionpalta_equipos-proteccion_ajs/" + formId + "/"
        : "/produccionpalta/produccionpalta_equipos-proteccion_ajs/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function editarProducto_utiles(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_utiles-oficina_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
...
function editarProducto_MaterialOficina(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_combustibles-lubricantes_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_computo(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_equipos-computo_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_otros_suministros(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_otros-suministros_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_construccion(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_material-construccion_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_repuestos(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_repuestos-accesorios_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_uit(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_equipos-uit_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_agricultura(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_materiales-agricultura_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_proteccion(id) {
  $.ajax({
    url: "/produccionpalta/produccionpalta_equipos-proteccion_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
