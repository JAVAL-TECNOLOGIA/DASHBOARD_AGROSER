          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerProduccionPalta.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/produccionpalta/objetivos_evaluacion/?campania="+ periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionpalta/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/produccionpalta/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerProduccionPalta = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/produccionpalta/objetivos_evaluacion/",
      detallesObjetivos: "/produccionpalta/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/PRODUCCION_PALTA/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - PRODUCCION PALTA",
      ...
      loading: {
        title: "Procesando - PRODUCCION PALTA",
        text: "Generando PDF desde PRODUCCION PALTA y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en PRODUCCION PALTA",
    ...
    metadata: {
      modulo: "PRODUCCION PALTA",
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - PRODUCCION PALTA",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar el módulo de gestión de correos electrónicos para PRODUCCION PALTA
  EmailManagerProduccionPalta.inicializar();
