...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/produccionpalta/produccionpalta_servicios_dl/?year=" + yearVal).load();
    return;
  }
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/produccionpalta/produccionpalta_servicios_dl/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/produccionpalta/produccionpalta_servicios_dl/?year="+ ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/produccionpalta/produccionpalta_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/produccionpalta/produccionpalta_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/produccionpalta/produccionpalta_servicios_dl/${id}/`
      : "/produccionpalta/produccionpalta_servicios_dl/",
