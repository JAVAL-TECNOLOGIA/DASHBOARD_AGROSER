...
function AGRICOLAAGRICOLAiniciarTablaResumen_SG(nombrePestaña) {
  ...
  if (nombrePestaña == "PLANTINES") {
    ...
    const tabla = $("#AGRICOLAAGRICOLAtablaFertilizacionPlantines_SG").DataTable({
      ...
      ajax: {
        url: "/produccionpalta/api/materia_agricola_palta/",
        type: "GET",
        data: function (d) {
          d.idfase = 1;
          d.idvariedad = 1;
      ...
      createdRow: function (row, data, dataIndex) {
        $(row).addClass("AGRICOLAprograma-row");
...
function AGRICOLAguardarNuevoPrograma_SG() {
  ...
  if (!hayErrores) {
    ...
    const url = esEdicion
      ? `/produccionpalta/api/materia_agricola_palta/${idregistro}/`
      : "/produccionpalta/api/materia_agricola_palta/";
...
function AGRICOLAeditarFertilizacion_SG(idRegistro) {
  ...
  $.ajax({
    url: `/produccionpalta/api/materia_agricola_palta/${idRegistro}/`,
    type: "GET",
    success: function (response) {
...
function AGRICOLAcargarProductosPrograma_AC(idPrograma) {
  ...
  function valorMesCantidad_AC(row, mes) {
    var k = mes + "_cantidad";
  ...
  function valorMesPrecio_AC(row, mes) {
    var k = mes + "_precio";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function totalFilaProducto_AC(row) {
    var suma = 0;
  ...
  mesesDetalle.forEach(function (mes) {
    columnasDetalle.push({
      ...
      className: "text-center",
      render: function (data, type, row) {
        return valorMesCantidad_AC(row, mes);
      },
    });
    columnasDetalle.push({
      data: null,
      className: "text-center",
      render: function (data, type, row) {
        return valorMesPrecio_AC(row, mes);
      },
    });
  });
  ...
  columnasDetalle.push({
    ...
    orderable: false,
    render: function (data, type, row) {
      return totalFilaProducto_AC(row);
    },
  });
  ...
  columnasDetalle.push({
    ...
    render: function (data, type, row) {
      var id = row.id;
      return (
        '<button type="button" class="btn btn-sm btn-info mr-1" title="Editar" onclick="editarProductoAutumnCrisp(' +
        id +
        ')"><i class="fas fa-edit"></i></button>' +
        '<button type="button" class="btn btn-sm btn-danger" title="Eliminar" onclick="eliminarProductoAutumnCrisp(' +
        id +
  ...
  $("#AGRICOLAtablaDetalleProductos_AC").DataTable({
    ...
    ajax: {
      url:
        "/produccionpalta/api/suministro_agricola_palta/?id_producto_agricola=" +
...
function editarProductoAutumnCrisp(idSuministro) {
  $.ajax({
    url: "/produccionpalta/api/suministro_agricola_palta/" + encodeURIComponent(idSuministro) + "/",
    type: "GET",
    success: function (response) {
      if (!response || !response.data) {
...
function eliminarProductoAutumnCrisp(idProducto) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url: `/produccionpalta/api/suministro_agricola_palta/${idProducto}/`,
        type: "DELETE",
        success: function (response) {
...
function AGRICOLAeliminarFertilizacion(idRegistro) {
  ...
  $.ajax({
    url: `/produccionpalta/api/suministro_agricola_palta/`,
    type: "GET",
    success: function (response) {
...
function AGRICOLAguardarProductoAutumnCrisp() {
  ...
  var url = "/produccionpalta/api/suministro_agricola_palta/";
  var metodo = "POST";
  if (esEdicion) {
    url = "/produccionpalta/api/suministro_agricola_palta/" + encodeURIComponent(suministroPk) + "/";
    metodo = "PUT";
  }
...
function AGRICOLAcargarProductosPrograma(idPrograma) {
  ...
  function valorMesCantidad(row, mes) {
    var k = mes + "_cantidad";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function valorMesPrecio(row, mes) {
    var k = mes + "_precio";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function totalFilaProducto(row) {
    var suma = 0;
    mesesDetalle.forEach(function (m) {
      var k = m + "_precio";
      if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
  ...
  mesesDetalle.forEach(function (mes) {
    columnasDetalle.push({
      ...
      className: "text-center",
      render: function (data, type, row) {
        return valorMesCantidad(row, mes);
      },
    });
    columnasDetalle.push({
      data: null,
...
function AGRICOLA_inicializarAutocompleteSuministroSG() {
  ...
  $("#AGRICOLA_descripcion_sum").autocomplete({
    ...
    source: function (request, response) {
      $.ajax({
        url: "/produccionpalta/api_productos_agricolas_palta/",
        dataType: "json",
        data: { q: request.term },
...
function AGRICOLA_inicializarAutocompleteSuministro_AC() {
  ...
  $("#AGRICOLA_descripcion_sum_AC").autocomplete({
    ...
    source: function (request, response) {
      $.ajax({
        url: "/produccionpalta/api_productos_agricolas_uva1/",
        dataType: "json",
        data: { q: request.term },
        success: function (data) {
          if (!data || data.error) {
...
$(document).ready(function () {
  ...
  // El evento para guardar producto ahora se maneja en AGRICOLAinitEventosFormularioProducto_SG
...
function AGRICOLAcargarProductosPrograma_MC(idPrograma) {
  ...
  function valorMesCantidad_MC(row, mes) {
    var k = mes + "_cantidad";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function valorMesPrecio_MC(row, mes) {
    var k = mes + "_precio";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function totalFilaProducto_MC(row) {
    var suma = 0;
    mesesDetalle.forEach(function (m) {
      var k = m + "_precio";
      if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
  ...
  mesesDetalle.forEach(function (mes) {
    columnasDetalle.push({
      ...
      className: "text-center",
      render: function (data, type, row) {
        return valorMesCantidad_MC(row, mes);
      },
    });
    columnasDetalle.push({
      data: null,
      className: "text-center",
      render: function (data, type, row) {
        return valorMesPrecio_MC(row, mes);
      },
    });
  });
  ...
  columnasDetalle.push({
    ...
    orderable: false,
    render: function (data, type, row) {
      return totalFilaProducto_MC(row);
    },
  });
  ...
  columnasDetalle.push({
    ...
    render: function (data, type, row) {
      var id = row.id;
      return (
        '<button type="button" class="btn btn-sm btn-info mr-1" title="Editar" onclick="editarProductoMoscatel(' +
        id +
        ')"><i class="fas fa-edit"></i></button>' +
        '<button type="button" class="btn btn-sm btn-danger" title="Eliminar" onclick="eliminarProductoMoscatel(' +
        id +
        ')"><i class="fas fa-trash"></i></button>'
      );
    },
...
// Función para editar una fila de SUMINISTRO_AGRICOLA (Sweet Globe)
function editarProductoMoscatel(idSuministro) {
  $.ajax({
    url: "/produccionpalta/api/suministro_agricola_palta/" + encodeURIComponent(idSuministro) + "/",
    type: "GET",
    success: function (response) {
...
function AGRICOLAcargarProductosPrograma_SUGRA(idPrograma) {
  ...
  function valorMesCantidad_SUGRA(row, mes) {
    var k = mes + "_cantidad";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function valorMesPrecio_SUGRA(row, mes) {
    var k = mes + "_precio";
    if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
      return parseFloat(row[k]).toFixed(2);
    }
  ...
  function totalFilaProducto_SUGRA(row) {
    var suma = 0;
    mesesDetalle.forEach(function (m) {
      var k = m + "_precio";
      if (row[k] != null && row[k] !== "" && !isNaN(parseFloat(row[k]))) {
  ...
  mesesDetalle.forEach(function (mes) {
    columnasDetalle.push({
      ...
      className: "text-center",
      render: function (data, type, row) {
        return valorMesCantidad_SUGRA(row, mes);
      },
    });
    columnasDetalle.push({
      data: null,
      className: "text-center",
      render: function (data, type, row) {
        return valorMesPrecio_SUGRA(row, mes);
      },
    });
  });
  ...
  columnasDetalle.push({
    ...
    orderable: false,
    render: function (data, type, row) {
      return totalFilaProducto_SUGRA(row);
    },
  });
...
function AGRICOLAcargarLotes(idVariedad, selectorId = null, idCampania = null, idEmpresa = null) {
  ...
  return new Promise((resolve, reject) => {
    ...
    $.ajax({
      url: "/produccionpalta/api/lotes_variedad/",
      type: "GET",
      data: {
        idvariedad: idVariedad,
...
function actualizarEstadisticasPlantines_SG(datos) {
  ...
  const procesarProgramas = async () => {
    ...
    $("#AGRICOLAprecioTotalSG_PLANTINES").text(formatearMoneda(precioTotalPorLtKg));
    $("#AGRICOLAcostoTotalSG_PLANTINES").text(formatearMoneda(precioTotalPorHa));
  };
