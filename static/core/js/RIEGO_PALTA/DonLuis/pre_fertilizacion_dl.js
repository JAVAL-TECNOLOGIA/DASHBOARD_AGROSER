...
function iniciarTablaResumen_SG(nombrePestaña) {
  ...
  if (nombrePestaña == "PLANTINES") {
    ...
    const tabla = $("#tablaFertilizacionPlantines_SG").DataTable({
      ...
      ajax: {
        url: "/riegopalta/api/riego_presupuesto_fertilizacion_dl/",
        type: "GET",
        data: function (d) {
          d.idfase = 1;
          d.idvariedad = 1;
...
function guardarNuevoPrograma_SG() {
  ...
  if (!hayErrores) {
    ...
    const url = esEdicion
      ? `/riegopalta/api/riego_presupuesto_fertilizacion_dl/${idregistro}/`
      : "/riegopalta/api/riego_presupuesto_fertilizacion_dl/";
...
function editarFertilizacion_SG(idRegistro) {
  ...
  $.ajax({
    url: `/riegopalta/api/riego_presupuesto_fertilizacion_dl/${idRegistro}/`,
    type: "GET",
    success: function (response) {
...
function cargarProductosPrograma_AC(idPrograma) {
  ...
  const tabla = $("#tablaDetalleProductos_AC").DataTable({
    ...
    ajax: {
      url: `/riegopalta/api/riego_dprefertilizacion/`,
      type: "GET",
      dataSrc: function (json) {
...
function guardarProductoAutumnCrisp() {
  ...
  let url = "/riegopalta/api/riego_dprefertilizacion/";
  let tipo = "POST";

  if (idProducto) {
    url = `/riegopalta/api/riego_dprefertilizacion/${idProducto}/`;
    tipo = "PUT";
  }
...
function guardarProductoSweetGlobe() {
  ...
  if (esEdicion) {
    url = `/riegopalta/api/riego_dprefertilizacion/${productoId}/`;
    metodo = "PUT";
  } else {
  }
