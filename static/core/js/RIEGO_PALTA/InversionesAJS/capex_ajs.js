...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/riegopalta/produccionpalta_capex_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/riegopalta/produccionpalta_capex_ajs/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/riegopalta/produccionpalta_capex_ajs/",
      dataSrc: "data",
    },
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "riegopalta/produccionpalta_capex_ajs");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/riegopalta/produccionpalta_capex_ajs/" + formId + "/"
        : "/riegopalta/produccionpalta_capex_ajs/",
...
function editarProducto_capex(id) {
  ...
  $.ajax({
    url: "/riegopalta/produccionpalta_capex_ajs/" + id + "/",
    type: "GET",
    success: function (data) {
