...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/riegopalta/produccionpalta_capex_cv/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/riegopalta/produccionpalta_capex_cv/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/riegopalta/produccionpalta_capex_cv/",
      dataSrc: "data",
    },
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "riegopalta/produccionpalta_capex_cv");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/riegopalta/produccionpalta_capex_cv/" + formId + "/"
        : "/riegopalta/produccionpalta_capex_cv/",
...
function editarProducto_capex(id) {
  ...
  $.ajax({
    url: "/riegopalta/produccionpalta_capex_cv/" + id + "/",
    type: "GET",
    success: function (data) {
