...
function calcularTotalPresupuesto_Capex() {
  ...
  $.ajax({
    url: "/contabilidad/contabilidad_capex_totals_dl/",
    type: "GET",
    dataType: "json",
    success: function (data) {
      // Actualizar totales por mes
...
function initCapexTable() {
  ...
  var table = $("#capexTable").DataTable({
    ...
    ajax: {
      url: "/contabilidad/contabilidad_capex/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
 ...
 */
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/contabilidad/contabilidad_capex/" + formId + "/"
        : "/contabilidad/contabilidad_capex/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
