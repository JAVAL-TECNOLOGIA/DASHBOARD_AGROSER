...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
    ...
    success: function (response) {
      if (response.status === "success") {
        ...
        if (!response.data || !response.data.id_evaluacion) {
          ...
          $.ajax({
            url: "/contabilidad/objetivos_evaluacion/",
            type: "GET",
...
const EmailManager = {
  ...
  config: {
    ...
    messages: {
      confirmation: {
        title: "Confirmación - CONTABILIDAD",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de CONTABILIDAD?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - CONTABILIDAD",
        text: "Generando PDF desde CONTABILIDAD y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en CONTABILIDAD",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - CONTABILIDAD",
    },
  },
