...
$(document).ready(function() {
    ...
    $('#filtroCampaniaDesempeno').on('change', function() {
        if (tablaEvaluacionesIntermedio) {
            // Obtener el período del filtro, y extraer solo el año si viene en formato CAMP2026
            let periodo = $('#filtroCampaniaDesempeno').val() || 'CAMP' + new Date().getFullYear();
            periodo = periodo.startsWith('CAMP') ? periodo.replace('CAMP', '') : periodo;
            
            // Recargar la tabla usando ajax.reload() en lugar de cambiar la URL manualmente
            tablaEvaluacionesIntermedio.ajax.reload();
        }
    });
...
function inicializarTablaEvaluacionesIntermedio() {
    tablaEvaluacionesIntermedio = $("#tablaEvaluaciones_intermedio").DataTable({
        ...
        buttons: [
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    dt.ajax.reload(null, false); // recarga sin resetear la paginación
                    setTimeout(() => {
        ...
        // Configuración Ajax
        ajax: {
            url: '/contabilidad/api/fase-intermedia/', // URL base fija
            type: 'GET',
            data: function(d) {
            ...
            dataSrc: function(json) {
                if (json.status === 'success') {
                    ...
                    mostrarMensaje('Error al cargar datos: ' + (json.message || 'Error desconocido'), 'error');
                    return [];
                }
            },
            error: function(xhr, error, thrown) {
                if (error === 'abort') return; // ignorar abort
                mostrarMensaje('Error de conexión al cargar datos - final', 'error');
        ...
        columns: [
            ...
            { data: 'porcentaje_general', title: 'Avance General', className: 'text-center', render: renderAvanceGeneral },
            { data: 'estado_avance', title: 'Estado', render: renderEstado },
            { data: null, title: 'Acciones', orderable: false, className: 'text-center', render: renderAcciones }
        ],
...
// Renderizadores
//====================
function renderEvaluado(data, type, row) {
...
function renderEvaluador(data, type, row) {
...
function renderObjetivos(data, type, row) {
    const total = row.total_objetivos || 0;
    const porcentaje = row.porcentaje_objetivos || 0;
    let badgeClass = 'badge-secondary', progressClass='bg-secondary';
    if(porcentaje>=75){badgeClass='badge-success';progressClass='bg-success'}
    else if(porcentaje>=50){badgeClass='badge-info';progressClass='bg-info'}
    else if(porcentaje>=25){badgeClass='badge-warning';progressClass='bg-warning'}
    else if(porcentaje>0){badgeClass='badge-danger';progressClass='bg-danger'}
    return `<span class="badge ${badgeClass}">${total} Objetivos</span>
...
function renderCompetencias(data,type,row){
    const total = row.total_competencias || 0;
    ...
    return `<span class="badge ${badgeClass}">${total} Competencias</span>
        <div class="progress mt-1" style="height: 5px;">
            <div class="progress-bar ${progressClass}" style="width: ${porcentaje}%"></div>
        </div>
        <small class="text-muted">${porcentaje}% completado</small>`;
...
function renderAvanceGeneral(data,type,row){
    const porcentaje = data || 0;
    let progressClass='bg-secondary';
    if(porcentaje>=75) progressClass='bg-gradient-success';
    else if(porcentaje>=50) progressClass='bg-gradient-primary';
    else if(porcentaje>=25) progressClass='bg-gradient-warning';
    else if(porcentaje>0) progressClass='bg-gradient-danger';
...
function renderEstado(data,type,row){
    ...
    switch(data){
        ...
        case 'Pendiente': badgeClass='badge-info'; break;
        case 'Sin Datos': badgeClass='badge-secondary'; break;
    }
...
function renderAcciones(data,type,row){
    return `<div class="btn-group" role="group">
        <button type="button" class="btn btn-primary btn-xs" onclick="abrirModalSeguimientoAvance(${row.id_evaluacion})" title="Registrar Avance">
            <i class="fas fa-edit"></i>
        </button>
        <button type="button" class="btn btn-info btn-xs" onclick="EmailManagerConta.generarEnviarPDFEvaluacion(${row.id_evaluacion})" title="Ver Detalles">
...
function cargarDatosSeguimientoAvance(id_evaluacion) {
    // Obtener la campaña seleccionada del filtro
    var periodo = $('#filtroCampaniaDesempeno').val() || 'CAMP' + new Date().getFullYear();
    // Extraer solo el año si viene en formato CAMP2025
    periodo = periodo.startsWith('CAMP') ? periodo.replace('CAMP', '') : periodo;
    
    $.ajax({
        url: `/contabilidad/api/fase-intermedia/?id_area=1&campania=${periodo}`,
        type: 'GET',
        dataType: 'json',
...
function cargarComentariosExistentes(id_evaluacion) {
    ...
    $.ajax({
        url: `/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&cargar_comentarios=true`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function cargarTablaObjetivosAvance(id_evaluacion) {
    ...
    $.ajax({
        url: `/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&tipo=1`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function cargarTablaCompetenciasAvance(id_evaluacion) {
    ...
    $.ajax({
        url: `/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&tipo=2`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
