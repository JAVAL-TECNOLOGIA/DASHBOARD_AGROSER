...
// TABLA DATATABLE
function calcularTotalPresupuesto_Capex() {
  // Mostrar total completo sin filtrar por año
  $.ajax({
    url: "/contabilidad/contabilidad_capex_totals_cv/",
    type: "GET",
    dataType: "json",
    success: function (data) {
      // Actualizar totales por mes
      ...
      meses.forEach((mes) => {
        ...
        const cantidad = data[`Total_${mes}_Cantidad`] || 0;
        const precio = parseFloat(data[`Total_${mes}_Precio`] || 0).toFixed(2);
      ...
      // Actualizar total general con formato de moneda peruana
      const totalGeneral = parseFloat(data.Total_General || 0);
      const montoFormateado = formatearMonedaPEN(totalGeneral);
    ...
    error: function (xhr, status, error) {
      ...
      console.log("Error al obtener el total de CAPEX:", error);
    },
  });
}
...
function initCapexTable() {
  ...
  var table = $("#capexTable").DataTable({
    ...
    ajax: {
      url: "/contabilidad/contabilidad_capex_cv/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "contabilidad/contabilidad_capex_cv");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/contabilidad/contabilidad_capex/" + formId + "/" : "/contabilidad/contabilidad_capex/",
 ...
 */
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/contabilidad/contabilidad_capex_cv/" + formId + "/"
        : "/contabilidad/contabilidad_capex_cv/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function editarProducto_capex(id) {
  $.ajax({
    url: "/contabilidad/contabilidad_capex/" + id + "/",
    type: "GET",
    success: function (data) {
...
function editarProducto_capex(id) {
  ...
  $.ajax({
    url: "/contabilidad/contabilidad_capex_cv/" + id + "/",
    type: "GET",
    success: function (data) {
