if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  function actualizarTotalSueldos() {
    ...
    $.ajax({
      url: "/contabilidad/contabilidad_sueldo_mensual/?year=" + year,
      type: "GET",
      dataType: "json",
    ...
    $.ajax({
      url: "/contabilidad/contabilidad_salario_mensual/",
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
        ...
        if (totalAnual) {
          ...
          console.log("No se encontró el total anual de los salarios");
        }
      },
    });
  }
  ...
  function initPresupuestoSueldos() {
    ...
    obtenerTasaCargaSocial().then(function () {
      ...
      tableSueldos = $("#sueldosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              dt.ajax.url("/contabilidad/contabilidad_sueldos_dl/?year=" + currentYear).load(function () {
        ...
        ajax: {
          url: "/contabilidad/contabilidad_sueldos_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
    ...
    $("#formSueldoSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/contabilidad/contabilidad_sueldos_dl/${formId}/`
            : "/contabilidad/contabilidad_sueldos_dl/",
  ...
  function initPresupuestoSalarios() {
    ...
    obtenerTasaCargaSocial_salarios().then(function () {
      tableSalarios = $("#salariosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              // Recargar la tabla con el filtro
              dt.ajax.url("/contabilidad/contabilidad_salarios_dl/?year=" + currentYear).load(function () {
                // Callback después de la recarga
                setTimeout(() => {
                  $(node).find("i").removeClass("fa-spin");
                }, 1000);
        ...
        ajax: {
          url: "/contabilidad/contabilidad_salarios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
        ...
        footerCallback: function (row, data, start, end, display) {
          ...
          // Si no hay datos, limpiar el footer y salir
          if (!data || data.length === 0) {
            const meses = ['enero', 'febrero', 'marzo', 'abril', 'mayo', 'junio', 
                          'julio', 'agosto', 'septiembre', 'octubre', 'noviembre', 'diciembre'];
            meses.forEach(function(mes) {
              footer.find('th[data-mes="' + mes + '"]').html('S/. 0.00');
            });
            footer.find('th[data-total="general"]').html('S/. 0.00');
            return;
          }
          ...
          meses.forEach(function(mes) {
            ...
            data.forEach(function(rowData) {
              const valor = parseFloat(rowData[mes]) || 0;
              // Calcular con carga social
              const asignacion = rowData.asignacion === "1" ? (0.1 * sueldoMinimo) : 0;
              const cargaSocial = (valor + asignacion) * tasaCargaSocialSalario;
              totalMes += valor + cargaSocial + asignacion;
            });
            footer.find('th[data-mes="' + mes + '"]').html(formatearMonto(totalMes));
            totalGeneral += totalMes;
          });
          
          footer.find('th[data-total="general"]').html(formatearMonto(totalGeneral));
        },
      });
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/contabilidad/contabilidad_salarios_dl/${formId}/`
            : "/contabilidad/contabilidad_salarios_dl/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
  ...
  function eliminarSueldo(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/contabilidad/contabilidad_sueldos_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
  ...
  function eliminarSalario(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/contabilidad/contabilidad_salarios_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
              Swal.fire("¡Eliminado!", response.message, "success");
