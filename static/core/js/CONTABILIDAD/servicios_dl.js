...
function initServiciosTable() {
  var table = $("#serviciosTable").DataTable({
    ...
    ajax: {
      url: "/contabilidad/contabilidad_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/contabilidad/contabilidad_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/contabilidad/contabilidad_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/contabilidad/contabilidad_servicios_dl/${id}/`
      : "/contabilidad/contabilidad_servicios_dl/",
