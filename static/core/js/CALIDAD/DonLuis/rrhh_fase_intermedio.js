                    ...
                    return `
                        <div class="btn-group" role="group">
                            ...
                            <button type="button" class="btn btn-info btn-xs" 
                                    onclick="EmailManagerCalidad.generarEnviarPDFEvaluacion(${row.id_evaluacion})" 
                                    title="Ver Detalles">
...
function cargarDatosSeguimientoAvance(id_evaluacion) {
    ...
    console.log("Cargando datos para evaluación:", id_evaluacion, "Campaña:", campania);
        ...
        success: function(response) {
            ...
            if (response.status === 'success' && response.data) {
                ...
                if (datosFiltrados.length > 0) {
                    ...
                    console.error("No se encontró la evaluación con ID:", id_evaluacion);
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'No se encontraron datos para esta evaluación'
                    });
                }
            } else {
...
function mostrarInformacionEvaluado(datos) {
    ...
    if (datos.length > 0) {
        ...
        // Actualizar información del evaluado
        $("#evaluado_periodo").text(evaluado.periodo || '- -');
        $("#nombre").text((evaluado.nombre_evaluado || '') + ' ' + (evaluado.apellido_evaluado || ''));
        $("#evaluado_cargo").text(evaluado.cargo_evaluado || 'Sin cargo');
        $("#evaluado_area").text(evaluado.nombre_area || 'Sin área');
        $("#evaluador_nombre").text((evaluado.nombre_evaluador || '') + ' ' + (evaluado.apellido_evaluador || ''));
        
        // Mostrar avatar con iniciales
        const nombreCompleto = (evaluado.nombre_evaluado || '') + ' ' + (evaluado.apellido_evaluado || '');
        const iniciales = nombreCompleto.trim().split(" ").map(n => n.charAt(0)).join("").substring(0, 2).toUpperCase() || '--';
        $(".modern-avatar .initials").text(iniciales);
        
        // Mostrar avance general
        const porcentajeGeneral = evaluado.porcentaje_general || 0;
        $("#avance_general").css("width", porcentajeGeneral + "%").html("<strong>" + porcentajeGeneral + "%</strong>");
        
        // Mostrar contadores de pestañas
        $("#count_objetivos").text(evaluado.total_objetivos || 0);
        $("#count_competencias").text(evaluado.total_competencias || 0);
