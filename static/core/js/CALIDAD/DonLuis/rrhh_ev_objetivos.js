          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerCalidad.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/calidad/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/calidad/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/calidad/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerCalidad = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/calidad/objetivos_evaluacion/",
      detallesObjetivos: "/calidad/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/CALIDAD/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - CALIDAD",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de CALIDAD?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - CALIDAD",
        text: "Generando PDF desde CALIDAD y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en CALIDAD",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - CALIDAD",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  EmailManagerCalidad.inicializar();
