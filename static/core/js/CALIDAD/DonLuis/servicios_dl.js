...
function initServiciosTable() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/calidad/calidad_servicios_dl/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/calidad/calidad_servicios_dl/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/calidad/calidad_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  ...
  $.ajax({
    url: `/calidad/calidad_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/calidad/calidad_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const formData = {
    ...
    ID_CAMPANIA: $("#filtroAnioPresupuesto").val()
  };
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/calidad/calidad_servicios_dl/${id}/`
      : "/calidad/calidad_servicios_dl/",
