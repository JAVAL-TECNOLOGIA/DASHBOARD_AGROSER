...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/rrhh/rrhh_capex_cv/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/rrhh/rrhh_capex_cv/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/rrhh/rrhh_capex_cv/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "rrhh/rrhh_capex_cv");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/rrhh/rrhh_capex_cv/" + formId + "/" : "/rrhh/rrhh_capex_cv/",
