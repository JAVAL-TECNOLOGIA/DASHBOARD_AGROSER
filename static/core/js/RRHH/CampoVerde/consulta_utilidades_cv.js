$(document).ready(function () {
  ...
  function verificarUtilidades(dni) {
    ...
    $.ajax({
      url: `/rrhh/datos_personal_por_dni_cv/?dni=${dni}`,
      type: "GET",
      dataType: "json",
      success: function (personalResponse) {
        if (personalResponse.status === "success" && personalResponse.data) {
          ...
          $.ajax({
            url: `/rrhh/certificado_utilidades_cv/?periodo=202504&factor=1&dni=${dni}`,
            type: "GET",
            dataType: "json",
  ...
  function solicitarFechaNacimiento(dni) {
    ...
    function verificarFechaNacimiento(fechaIngresada) {
      ...
      $.ajax({
        url: `/rrhh/personal_general_cv/?dni=${dni}`,
        type: "GET",
        dataType: "json",
  ...
  // Validación para la cuenta interbancaria
  $('input[name="cuentaInterbancaria"]').on("input", function () {
    const valor = $(this).val().trim();
    const numerosOnly = valor.replace(/\D/g, ""); // Quitar cualquier carácter que no sea número

    if (numerosOnly.length > 20) {
      $("#cuentaInterbancariaHelp").show();
      $(this).css("border-color", "#e74c3c");
    } else {
      $("#cuentaInterbancariaHelp").hide();
      $(this).css("border-color", "#e4e6ef");
    }
  });

  // Añadir validación adicional al enviar el formulario
  $("#userInfoForm").submit(function (e) {
    e.preventDefault();

    // Validación del número de cuenta interbancaria
    const cuentaInterbancaria = $('input[name="cuentaInterbancaria"]')
      .val()
      .trim();
    const numerosOnly = cuentaInterbancaria.replace(/\D/g, "");

    if (numerosOnly.length > 20) {
      Swal.fire({
        title: "Advertencia",
        text: "La cuenta interbancaria ingresada excede los 20 dígitos. ¿Está seguro que es correcta?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Sí, es correcta",
        cancelButtonText: "No, revisaré",
      }).then((result) => {
        if (result.isConfirmed) {
  ...
  // Función separada para validar y enviar el formulario
  function validarYEnviarFormulario() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          url: "/rrhh/utilidades_empleados_cv/",
          type: "POST",
          ...
          error: function (xhr, status, error) {
            ...
            Swal.fire({
              icon: "error",
              title:
                "Ocurrió un error verifique los datos o contacte con Recursos Humanos",
              //text: errorMessage,
              confirmButtonText: "Entendido",
            });
          },
