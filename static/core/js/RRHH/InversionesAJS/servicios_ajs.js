...
function initServiciosTable() {
   ...
   */
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      // Cargar los totales desde la API
      $.ajax({
        url: "/rrhh/rrhh_servicios_totals_ajs/",
        type: "GET",
        success: function (response) {
          const meses = [
            "Enero",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/rrhh/rrhh_servicios_ajs/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit ? `/rrhh/rrhh_servicios_ajs/${id}/` : "/rrhh/rrhh_servicios_ajs/",
