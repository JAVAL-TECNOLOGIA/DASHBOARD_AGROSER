...
// ============================================================================
// FUNCION PRINCIPAL: Inicializa la tabla de CAPEX
...
function initCapexTable() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    $("#capexTable").DataTable().ajax.url("/rrhh/rrhh_capex_ajs/?year=" + yearVal).load();
    return;
  }
  ...
  var table = $("#capexTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/rrhh/rrhh_capex_ajs/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/rrhh/rrhh_capex_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      // Calcular total general sumando todos los precios mensuales
      tableData.forEach(function(row) {
        meses.forEach(function(mes) {
          totalGeneral += parseFloat(row[mes + "_precio"]) || 0;
        });
      });
  ...
  $("#capexTable tbody").on("click", ".eliminar-capex", function () {
    var id = $(this).data("id");
    eliminarProducto_capex(id, "rrhh/rrhh_capex_ajs");
  });
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/rrhh/rrhh_capex_ajs/" + formId + "/" : "/rrhh/rrhh_capex_ajs/",
