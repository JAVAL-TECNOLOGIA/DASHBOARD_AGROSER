...
function initServiciosTableApli() {
  ...
  if ($.fn.DataTable.isDataTable("#serviciosTable")) {
    $("#serviciosTable").DataTable().ajax.url("/aplicaciones/aplicaciones_servicios_dl/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          // Recargar la tabla
          dt.ajax.url("/aplicaciones/aplicaciones_servicios_dl/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/aplicaciones/aplicaciones_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/aplicaciones/aplicaciones_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/aplicaciones/aplicaciones_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/aplicaciones/aplicaciones_servicios_dl/${id}/`
      : "/aplicaciones/aplicaciones_servicios_dl/",
