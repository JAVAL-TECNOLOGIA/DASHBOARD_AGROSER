...
function inicializarTablaEvaluaciones_competencia() {
  var table = $("#tablaEvaluaciones_competencias").DataTable({
    ...
    ajax: {
      url: "/aplicaciones/resumen_competencias/",
      type: "GET",
      data: function (d) {
...
function inicializarTablaDetallesCompetencias(id_evaluacion) {
  ...
  var table = $("#tablaDetallesCompetencias").DataTable({
    ...
    ajax: {
      url: "/aplicaciones/detalles_competencias/",
      type: "GET",
...
function consultarDetallesEvaluacion(id_evaluacion) {
  ...
  $.ajax({
    url: "/aplicaciones/detalles_objetivos/",
    type: "GET",
    dataType: "json",
  ...
  try {
    ...
    if (objetivos && objetivos.length > 0) {
      objetivos.forEach((objetivo, index) => {
        ...
        // Comentarios
        pdf.rect(10, yPos, (pdfWidth - 20) / 2, 30, "S");
        pdf.rect(10 + (pdfWidth - 20) / 2, yPos, (pdfWidth - 20) / 2, 30, "S");
        pdf.text(`Comentario: ${datosEvaluado.comentario_objetivo } `, 12, yPos + 5);
        pdf.text("Comentario: ", 10 + (pdfWidth - 20) / 2 + 2, yPos + 5);
    ...
    if (competencias && competencias.length > 0) {
      competencias.forEach((competencia, index) => {
        ...
        pdf.text(`Comentario: ${datosEvaluado.comentario_competencia}`, 12, yPos + 5);
        pdf.text("Comentario: ", 10 + (pdfWidth - 20) / 2 + 2, yPos + 5);
...
function cargarEvaluados() {
  $.ajax({
    url: "/aplicaciones/objetivos_evaluacion/",
    type: "GET",
    success: function (response) {
      if (response.status === "success") {
...
function editarCompetencia(id) {
  $.ajax({
    url: `/rrhh/detalles_competencias/${id}/`,
    type: "GET",
    success: function (response) {
      if (response.status === "success" && response.data.length > 0) {
...
function guardarEdicionCompetencia() {
  ...
  // Enviar datos al servidor
  $.ajax({
    url: `/aplicaciones/detalles_competencias/${competenciaId}/`,
    type: "PUT",
    contentType: "application/json",
...
function eliminarCompetencia(id) {
  $.when(
    ...
    if (result.isConfirmed) {
      ...
      $.ajax({
        url: `/aplicaciones/detalles_competencias/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": getCookie("csrftoken"),
        },
...
/* document.addEventListener("DOMContentLoaded", function () {
  $("#modalCompetencias").on("shown.bs.modal", function () {
    // Verificar si hay un ID de evaluación guardado
    const preselectedEvaluacionId = localStorage.getItem(
      "preselectedEvaluacionId"
    );
    ...
    if (preselectedEvaluacionId) {
      console.log(
        "Modal mostrado, buscando evaluación:",
        preselectedEvaluacionId
      );

      // Intentar seleccionar directamente
      $("#evaluadoSelect").val(preselectedEvaluacionId);

      // Si no funciona, recargar evaluados
      if (!$("#evaluadoSelect").val()) {
        cargarEvaluados();
      } else {
        // Si funciona, disparar el evento change
        $("#evaluadoSelect").trigger("change");
        // Y limpiar el localStorage
        localStorage.removeItem("preselectedEvaluacionId");
      }
    }
  });

  // Evento personalizado para cuando se solicita específicamente
  $(document).on(
    "modalCompetenciasRequested",
    function (event, idEvaluado, nombreEvaluado) {
      console.log(
        "Evento modalCompetenciasRequested recibido:",
        idEvaluado,
        nombreEvaluado
      );

      // Asegurar que el modal esté completamente visible antes de intentar seleccionar
      $("#modalCompetencias").on("shown.bs.modal.autoSelect", function () {
        console.log(
          "Modal de competencias completamente visible, seleccionando evaluado..."
        );
        // Hacer una búsqueda más completa
        buscarYSeleccionarEvaluado(idEvaluado, nombreEvaluado);
        // Remover el handler para evitar múltiples ejecuciones
        $(this).off("shown.bs.modal.autoSelect");
      });
    }
  );
  ...
  $(document).on("click", "#btn_competencias", function () {
    cargarEvaluados();
  });

  $("#btn_competencias").on("click", function () {
    cargarEvaluados();
  });

  $("#guardarCompetencias").on("click", function () {
    guardarEvaluacion();
  });

  // Evento para el botón Cancelar
  $("#modalCompetencias .btn-secondary").on("click", function () {
    resetModalCompetencias();
  });

  // Evento para cuando se cierra el modal (X o haciendo clic fuera)
...
// Agregar al final del archivo document.addEventListener("DOMContentLoaded", function() { ... })
