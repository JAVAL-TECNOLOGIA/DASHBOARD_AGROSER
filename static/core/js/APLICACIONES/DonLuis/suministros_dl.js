...
function calcularTotalPresupuesto() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#materialOficinaTable")) {
      ...
      var totalGeneral = 0;
      var mesesPrecio = ['enero_precio', 'febrero_precio', 'marzo_precio', 'abril_precio', 
                        'mayo_precio', 'junio_precio', 'julio_precio', 'agosto_precio', 
                        'septiembre_precio', 'octubre_precio', 'noviembre_precio', 'diciembre_precio'];
      
      data.forEach(function(row) {
        mesesPrecio.forEach(function(mes) {
          totalGeneral += parseFloat(row[mes]) || 0;
        });
      });
      
      $("#totalPresupuestoValor").text(formatearMonedaPEN(totalGeneral));
      $("#stats_materialoficina").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_UtilesOficina() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#utilesOficinaTable")) {
      ...
      $("#totalPresupuestoValor_utiles").text(formatearMonedaPEN(totalGeneral));
      $("#stats_utilesoficina").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_EquiposComputo() {
  return new Promise((resolve, reject) => {
    // Calcular el total directamente de los datos de la tabla (ya filtrados por campaña)
    if ($.fn.DataTable.isDataTable("#equiposComputoTable")) {
      var table = $("#equiposComputoTable").DataTable();
      var data = table.rows().data().toArray();
      ...
      $("#totalPresupuestoValor_computo").text(formatearMonedaPEN(totalGeneral));
      $("#stats_equiposcomputo").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_OtrosSuministros() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#otrosSuministrosTable")) {
      ...
      $("#totalPresupuestoValor_otros").text(formatearMonedaPEN(totalGeneral));
      $("#stats_otrossuministros").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_MaterialConstruccion() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#materialConstruccionTable")) {
      ...
      $("#totalPresupuestoValor_construccion").text(formatearMonedaPEN(totalGeneral));
      $("#stats_materialesconstruccion").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_RepuestosAccesorios() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#repuestosAccesoriosTable")) {
      ...
      $("#totalPresupuestoValor_repuestos").text(formatearMonedaPEN(totalGeneral));
      $("#stats_repuestosaccesorios").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_EquiposUIT() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#equiposUITTable")) {
      ...
      $("#totalPresupuestoValor_uit").text(formatearMonedaPEN(totalGeneral));
      $("#stats_equiposuit").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_MaterialAgricultura() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#materialagriculturaTable")) {
      ...
      $("#totalPresupuestoValor_agricultura").text(formatearMonedaPEN(totalGeneral));
      $("#stats_materialesagricultura").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
function calcularTotalPresupuesto_EquiposProteccion() {
  return new Promise((resolve, reject) => {
    ...
    if ($.fn.DataTable.isDataTable("#equipoProteccionTable")) {
      ...
      $("#totalPresupuestoValor_proteccion").text(formatearMonedaPEN(totalGeneral));
      $("#stats_equiposproteccion").text(formatearMonedaPEN(totalGeneral));
      actualizarConsolidadoMateriales();
      resolve();
    } else {
      resolve();
...
// Inicializa la tabla de combustibles y lubricantes con filtro por campaña
// ============================================================================
...
function initPresupuestoMateriales() {
  ...
  if ($.fn.DataTable.isDataTable("#materialOficinaTable")) {
    $("#materialOficinaTable").DataTable().ajax.url("/aplicaciones/aplicaciones_combustibles-lubricantes/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#materialOficinaTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_combustibles-lubricantes", //+ yearVal,
      dataSrc: "data",
    },
    ...
    // destroy: true, // Removido para evitar reinicializacion
  });
  ...
  $("#registerMaterialForm").on("submit", function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_combustibles-lubricantes/" + formId + "/"
        : "/aplicaciones/aplicaciones_combustibles-lubricantes/",
...
function initPresupuestoUtilesOficina() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#utilesOficinaTable")) {
    $("#utilesOficinaTable").DataTable().ajax.url("/aplicaciones/aplicaciones_utiles-oficina/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#utilesOficinaTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_utiles-oficina/",// + yearVal,
      dataSrc: "data",
    },
    fixedColumns: {
      right: 1, // Fija la última columna (acciones) a la derecha
  ...
  $("#registerMaterialForm_utiles").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_utiles-oficina/" + formId + "/"
        : "/aplicaciones/aplicaciones_utiles-oficina/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposComputo() {
  ...
  if ($.fn.DataTable.isDataTable("#equipoComputoTable")) {
    $("#equipoComputoTable").DataTable().ajax.url("/aplicaciones/aplicaciones_equipos-computo/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#equipoComputoTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_equipos-computo/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
  ...
  $("#registerMaterialForm_computo").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_equipos-computo/" + formId + "/"
        : "/aplicaciones/aplicaciones_equipos-computo/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoOtrosSuministros() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#otrosSuministrosTable")) {
    $("#otrosSuministrosTable").DataTable().ajax.url("/aplicaciones/aplicaciones_otros-suministros/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#otrosSuministrosTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_otros-suministros/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#registerMaterialForm_otros").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_otros-suministros/" + formId + "/"
        : "/aplicaciones/aplicaciones_otros-suministros/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoMaterialConstruccion() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#materialConstruccionTable")) {
    $("#materialConstruccionTable").DataTable().ajax.url("/aplicaciones/aplicaciones_material-construccion/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#materialConstruccionTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_material-construccion/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#registerMaterialForm_construccion").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_material-construccion/" + formId + "/"
        : "/aplicaciones/aplicaciones_material-construccion/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoRepuestosAccesorios() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#repuestosAccesoriosTable")) {
    $("#repuestosAccesoriosTable").DataTable().ajax.url("/aplicaciones/aplicaciones_repuestos-accesorios/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#repuestosAccesoriosTable").DataTable({
    ...
    // Ajax para obtener los datos de la tabla filtrados por campaña
    ajax: {
      url: "/aplicaciones/aplicaciones_repuestos-accesorios/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
      { data: "id", title: "ID", visible: false },
  ...
  $("#registerMaterialForm_repuestos").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_repuestos-accesorios/" + formId + "/"
        : "/aplicaciones/aplicaciones_repuestos-accesorios/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposUIT() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#equiposuitTable")) {
    $("#equiposuitTable").DataTable().ajax.url("/aplicaciones/aplicaciones_equipos-uit/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#equiposuitTable").DataTable({
    ...
    ajax: {
      url: "/aplicaciones/aplicaciones_equipos-uit/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
  ...
  $("#registerMaterialForm_uit").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_equipos-uit/" + formId + "/"
        : "/aplicaciones/aplicaciones_equipos-uit/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoMaterialAgricultura() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#materialagriculturaTable")) {
    $("#materialagriculturaTable").DataTable().ajax.url("/aplicaciones/aplicaciones_materiales-agricultura/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#materialagriculturaTable").DataTable({
    ...
    ajax: {
      url: "/aplicaciones/aplicaciones_materiales-agricultura/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    fixedColumns: {
      right: 1, // Fija la última columna (acciones) a la derecha
  ...
  $("#registerMaterialForm_agricultura").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_materiales-agricultura/" + formId + "/"
        : "/aplicaciones/aplicaciones_materiales-agricultura/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function initPresupuestoEquiposProteccion() {
  ...
  // Si la tabla ya existe, solo recargar los datos con el nuevo filtro
  if ($.fn.DataTable.isDataTable("#equipoProteccionTable")) {
    $("#equipoProteccionTable").DataTable().ajax.url("/aplicaciones/aplicaciones_equipos-proteccion/?year=" + CAMPANIA_ACTIVA).load();
    return;
  }
  ...
  var table = $("#equipoProteccionTable").DataTable({
    ...
    ajax: {
      url: "/aplicaciones/aplicaciones_equipos-proteccion/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
    columns: [
      { data: "id", visible: false },
  ...
  $("#registerMaterialForm_proteccion").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/aplicaciones/aplicaciones_equipos-proteccion/" + formId + "/"
        : "/aplicaciones/aplicaciones_equipos-proteccion/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function editarProducto_utiles(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_utiles-oficina/" + id + "/",
    type: "GET",
    success: function (data) {
...
function editarProducto_MaterialOficina(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_combustibles-lubricantes/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_computo(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_equipos-computo/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_otros_suministros(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_otros-suministros/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_construccion(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_material-construccion/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_repuestos(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_repuestos-accesorios/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_uit(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_equipos-uit/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_agricultura(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_materiales-agricultura/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function editarProducto_proteccion(id) {
  $.ajax({
    url: "/aplicaciones/aplicaciones_equipos-proteccion/" + id + "/",
    type: "GET",
    success: function (data) {
      // Llenar el formulario con los datos del producto
...
function actualizarConsolidadoMateriales() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/aplicaciones/aplicaciones_suministros_totals_dl/?year=${yearVal}`,
    type: "GET",
    dataType: "json",
    success: function (data) {
...
function initPresupuesto(config) {
  ...
  return function () {
    ...
    const calcularTotalPresupuesto = () => {
      return new Promise((resolve, reject) => {
        $.ajax({
          url: "/aplicaciones/aplicaciones_suministros_totals_dl/",
          type: "GET",
          dataType: "json",
          success: function (data) {
            $(totalSelector).text(
