...
$(document).ready(function () {
  console.log("Inicializando Fase Final de GERENCIA...");
  inicializarTablaEvaluacionesFinal();

  // Inicializar eventos de los inputs de avance final
  if (typeof initializarEventosAvanceFinal === 'function') {
...
function inicializarTablaEvaluacionesFinal() {
  ...
  if ($.fn.DataTable.isDataTable("#tablaEvaluaciones_final")) {
    $("#tablaEvaluaciones_final").DataTable().destroy();
  }
    ...
    tablaEvaluacionesFinal = $("#tablaEvaluaciones_final").DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                // Asegurar que solo se envíe el año
                if (typeof campania === 'string' && campania.startsWith('CAMP')) {
        ...
        columns: [
            ...
            { 
                ...
                className: 'text-center',
                render: function(data, type, row) {
                    // Usar el porcentaje del Avance Intermedio si el Avance Final es 0 o no está definido
                    const porcentaje = data || row.porcentaje_intermedio || 0;
                    let progressClass = 'bg-secondary';
                    ...
                    return `
                        <div class="btn-group" role="group">
                            ...
                            <button type="button" class="btn btn-primary btn-sm" 
                                    onclick="EmailManagerGere.generarEnviarPDFEvaluacionFF(${row.id_evaluacion})" 
...
function cargarDatosSeguimientoAvanceFinal(id_evaluacion) {
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log('Response cargarDatosSeguimientoAvanceFinal /gerencia/api/fase-final/');
...
function cargarComentariosExistentesFinal(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion-final/?id_evaluacion=${id_evaluacion}&cargar_comentarios=true`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function cargarTablaObjetivosAvanceFinal(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion-final/?id_evaluacion=${id_evaluacion}&tipo=1`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
                        ...
                        <tr data-objetivo-final-id="${objetivo.id || index + 1}">
                            ...
                            <td>
                                <div class="progress" style="height: 15px;">
                                    <div class="progress-bar bg-primary progress-bar-striped progress-bar-animated" style="width: ${objetivo.porcentaje_actual || 0}%">
...
function cargarTablaCompetenciasAvanceFinal(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion-final/?id_evaluacion=${id_evaluacion}&tipo=2`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
                ...
                response.data.forEach(function(competencia, index) {
                    ...
                    filas += `
                        <tr data-competencia-final-id="${competencia.id || index + 1}">
                            ...
                            <td>
                                <div class="progress" style="height: 15px;">
                                    <div class="progress-bar bg-primary progress-bar-striped progress-bar-animated" style="width: ${competencia.porcentaje_actual || 0}%">
...
function actualizarAvancesFinalEnServidor(tipo, actualizaciones, tipoNombre, callback, comentarios = {}) {
    ...
    $.ajax({
        url: '/gerencia/api/detalles-evaluacion-final/',
        type: 'PUT',
...
// Redondear el valor de avanceFinal para evitar mostrar decimales excesivos
// const roundedAvanceFinal = Math.round(avanceFinal);
//             $('#avanceFinal').text(`${roundedAvanceFinal}%`);


// FASE FINAL - FIN

const EmailManagerGere = {
  // Configuración del módulo específica para Fase Final
  config: {
    endpoints: {
      evaluaciones: "/gerencia/objetivos_evaluacion/",
      detallesObjetivos: "/gerencia/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/GERENCIA/DonLuis/rrhh_ev_competencias.js",
        "/static/core/js/EVALUACIONES/DonLuis/rrhh_ev_competencias.js",
        "/static/core/js/CONTABILIDAD/DonLuis/rrhh_ev_competencias.js",
  ...
  esFuncionPDFDisponible() {
    return typeof generarPDFFinalGere === "function";
  },
  ...
  ejecutarGeneracionPDF(idEvaluacion) {
    if (this.esFuncionPDFDisponible()) {
      ...
      generarPDFFinalGere(idEvaluacion);
    } else {
  ...
  generarEnviarPDFEvaluacionFF(idEvaluacion) {
...
function generarPDFFinalGere(id_evaluacion) {
      ...
      $.ajax({
        ...
        success: function (responseUsuarios) {
          ...
          const continuarFlujo = () => {
            ...
            $.ajax({
              url: "/gerencia/detalles_objetivos/",
              type: "GET",
              dataType: "json",
              success: function (responseObjetivos) {
                ...
                $.ajax({
                  url: "/gerencia/detalles_competencias/",
                  type: "GET",
                  dataType: "json",
                  success: function (responseCompetencias) {
                    ...
                    crearContenidoPDFGere(datosCompletos, objetivos, competencias);
                  },
...
function crearContenidoPDFGere(datosEvaluado, objetivos, competencias) {
  try {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        enviarPDFPorCorreoGere(datosEvaluado, nombrePDF, pdfBlob);
      }
    });
  } catch (error) {
...
function enviarPDFPorCorreoGere(datosEvaluado, nombrePDF, pdfBlob) {
  Swal.fire({
