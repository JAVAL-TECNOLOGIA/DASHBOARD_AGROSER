...
$(document).ready(function() {
    console.log("Inicializando Fase Intermedia de GERENCIA...");
    inicializarTablaEvaluacionesIntermedio();
    
    // Inicializar eventos de los inputs de avance
    if (typeof initializarEventosAvance === 'function') {
        initializarEventosAvance();
    }
    
    // Inicializar eventos del modal rápido
    if (typeof initializarEventosAvanceRapido === 'function') {
...
function inicializarTablaEvaluacionesIntermedio() {
    ...
    tablaEvaluacionesIntermedio = $("#tablaEvaluaciones_intermedio").DataTable({
        ...
        ajax: {
            url: '/gerencia/api/fase-intermedia/',
            type: 'GET',
            data: function(d) {
                    ...
                    return `
                        <div class="btn-group" role="group">
                            ...
                            <button type="button" class="btn btn-success btn-sm" 
                                    onclick="EmailManagerGerencia.generarEnviarPDFEvaluacion(${row.id_evaluacion})" 
        ...
        // Configuraciones mejoradas para el manejo de columnas
        scrollX: true,
        ...
        drawCallback: function(settings) {
            // Recalcular columnas después de cada redibujado
            this.api().columns.adjust();
        },
...
function cargarComentariosExistentes(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&cargar_comentarios=true`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function cargarTablaObjetivosAvance(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&tipo=1`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function cargarTablaCompetenciasAvance(id_evaluacion) {
    ...
    $.ajax({
        url: `/gerencia/api/detalles-evaluacion/?id_evaluacion=${id_evaluacion}&tipo=2`,
        type: 'GET',
        dataType: 'json',
        success: function(response) {
...
function guardarTodoElAvance() {
    ...
    if (actualizacionesObjetivos.length > 0) {
        ...
        actualizarAvancesEnServidor(1, actualizacionesObjetivos, 'objetivos', function(success) {
            ...
            //comentarios_objetivos: comentariosObjetivos
        });
    }
...
function actualizarAvancesEnServidor(tipo, actualizaciones, tipoNombre, callback, comentarios = {}) {
    ...
    $.ajax({
        url: '/gerencia/api/detalles-evaluacion/',
        type: 'PUT',
