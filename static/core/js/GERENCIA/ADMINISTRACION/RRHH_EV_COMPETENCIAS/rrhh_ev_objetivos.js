...
function inicializarTablaEvaluaciones_objetivos() {
  var table = $("#tablaEvaluaciones_objetivos").DataTable({
   ...
   ajax: {
      url: "/gerencia/objetivos_evaluacion/",
      type: "GET",
      data: function (d) {
          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerGerencia.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/gerencia/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/gerencia/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/gerencia/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerGerencia = {
  ...
  config: {
    ...
    messages: {
      confirmation: {
        title: "Confirmación - GERENCIA",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de GERENCIA?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - GERENCIA",
        text: "Generando PDF desde GERENCIA y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en GERENCIA",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - GERENCIA",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  EmailManagerGerencia.inicializar();
