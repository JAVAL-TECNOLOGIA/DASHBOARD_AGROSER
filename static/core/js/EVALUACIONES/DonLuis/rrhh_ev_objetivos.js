          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerEvaluaciones.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/evaluaciones/objetivos_evaluacion/?campania="+ periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/evaluaciones/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/evaluaciones/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerEvaluaciones = {
  ...
  config: {
    ...
    messages: {
      confirmation: {
        title: "Confirmación - Evaluaciones",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de Evaluaciones?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - Evaluaciones",
        text: "Generando PDF desde Evaluaciones y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en Evaluaciones",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - EVALUACIONES",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  EmailManagerEvaluaciones.inicializar();
