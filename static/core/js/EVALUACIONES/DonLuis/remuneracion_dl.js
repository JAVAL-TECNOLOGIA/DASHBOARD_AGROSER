if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  $(document).ready(function () {
    ...
    // Cuando se abre el modal de sueldos - solo inicializar si no existe
    ...
    actualizarTotalSalarios();
    setTimeout(actualizarTotalSueldos, 200);
  });
  ...
  function actualizarTotalSueldos() {
    ...
    $.ajax({
      url: "/evaluaciones/evaluaciones_sueldo_mensual/?year=" + year,
      type: "GET",
      dataType: "json",
  ...
  function actualizarTotalSalarios() {
    const year = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
    $.ajax({
      url: "/evaluaciones/evaluaciones_salario_mensual/?year=" + year,
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
        ...
        if (totalAnual) {
          ...
          Swal.fire({
            title: "Error!",
            text: "No se encontró el total anual de los salarios",
            icon: "error",
          });
        }
      },
  ...
  //-------------------------------------------------------------------------------
  function esDniGenericoSueldo(dni) {
    ...
    $("#dni_sueldo_salario")
      ...
      .autocomplete({
        source: function (request, response) {
          if (esDniGenericoSueldo(request.term)) {
            response([]);
            return;
          }
          $.ajax({
        ...
        select: function (event, ui) {
          // Evitar duplicados
          if (!esDniGenericoSueldo($("#dni_sueldo_salario").val())) {
  ...
  function esDniGenericoSalario(dni) {
    return /^(\d)\1{7}$/.test(dni);
  }
  function inicializarAutocompleteDNI_Salario() {
    $("#dni_salario")
      ...
      .autocomplete({
        source: function (request, response) {
          if (esDniGenericoSalario(request.term)) {
            response([]);
            return;
          }
          $.ajax({
        ...
        minLength: 8,
        select: function (event, ui) {
          if (!esDniGenericoSalario($("#dni_sueldo_salario").val())) {
            let dniExistente = false;
        ...
        change: function (event, ui) {
          if (!esDniGenerico($("#dni_salario").val())) {
            limpiarCampos();
            Swal.fire({
              title: "Error!",
  ...
  function initPresupuestoSueldos() {
    ...
    obtenerTasaCargaSocial().then(function () {
      ...
      table = $("#sueldosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              // Recargar la tabla
              dt.ajax.url("/evaluaciones/evaluaciones_sueldos_dl/?year=" + currentYear).load(function () {
        ...
        ajax: {
          url: "/evaluaciones/evaluaciones_sueldos_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
        ...
        footerCallback: function (row, data, start, end, display) {
          ...
          $.ajax({
            url: "/evaluaciones/evaluaciones_sueldo_mensual/",
            type: "GET",
            success: function (response) {
              if (response.data && response.data.length > 0) {
                // Crear un mapa de los consolidados por mes
                ...
                response.data.forEach((item) => {
                  if (item.Mes !== "Total Anual") {
                    consolidadosPorMes[item.Mes.toLowerCase()] =
                      item.Consolidado_Mes;
                  } else {
                ...
                // Actualizar cada celda del footer con el consolidado correspondiente
                Object.keys(consolidadosPorMes).forEach((mes) => {
                  const consolidado = consolidadosPorMes[mes] || 0;
                  footer
                    .find(`th[data-mes="${mes.toLowerCase()}"]`)
                    .html(formatearMonto(consolidado));
                });
            ...
            error: function (xhr, status, error) {
              Swal.fire({
                title: "Error!",
                text: "Error al obtener los totales: " + error,
                icon: "error",
              });
            },
          });
    ...
    $("#formSueldoSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/evaluaciones/evaluaciones_sueldos_dl/${formId}/`
            : "/evaluaciones/evaluaciones_sueldos_dl/",
  ...
  function initPresupuestoSalarios() {
    ...
    if ($.fn.DataTable.isDataTable("#salariosTable")) {
      ...
      $("#salariosTable").DataTable().ajax.reload();
      return;
    }
    ...
    obtenerTasaCargaSocial_salarios().then(function () {
      tableSalarios = $("#salariosTable").DataTable({
        ...
        ajax: {
          url: "/evaluaciones/evaluaciones_salarios_dl/",
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
        ...
        footerCallback: function (row, data, start, end, display) {
          ...
          $.ajax({
            url: "/evaluaciones/evaluaciones_salario_mensual/",
            type: "GET",
            success: function (response) {
              if (response.data && response.data.length > 0) {
                // Crear un mapa de los consolidados por mes
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/evaluaciones/evaluaciones_salarios_dl/${formId}/`
            : "/evaluaciones/evaluaciones_salarios_dl/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
  ...
  function eliminarSueldo(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/evaluaciones/evaluaciones_sueldos_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
  ...
  function eliminarSalario(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/evaluaciones/evaluaciones_salarios_dl/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
              Swal.fire("¡Eliminado!", response.message, "success");
