          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerCostos.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/costos/objetivos_evaluacion/?campania=" + periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/costos/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/costos/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerCostos = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/costos/objetivos_evaluacion/",
      detallesObjetivos: "/costos/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
    ...
    messages: {
      confirmation: {
        title: "Confirmación - COSTOS",
        text: "¿Desea generar el PDF de esta evaluación y enviarlo por correo desde el módulo de COSTOS?",
        confirmButtonText: "Sí, generar y enviar",
        cancelButtonText: "Cancelar",
      },
      loading: {
        title: "Procesando - COSTOS",
        text: "Generando PDF desde COSTOS y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en COSTOS",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - COSTOS",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  EmailManagerCostos.inicializar();
