...
function initServiciosTable() {
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/costos/costo_servicios_dl/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/costos/costo_servicios_dl/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/costos/costo_servicios_totals_dl/?year=${yearVal}`,
    type: "GET",
    success: function (response) {
...
$(document).on("click", ".eliminar-servicio", function () {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/costos/costo_servicios_dl/${id}/`,
        type: "DELETE",
        success: function (response) {
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/costos/costo_servicios_dl/${id}/`
      : "/costos/costo_servicios_dl/",
