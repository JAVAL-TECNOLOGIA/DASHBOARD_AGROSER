          ...
          return `
                <div class="btn-group btn-group-sm" role="group" style="display: flex; justify-content: center;">
                    <button type="button" 
                            ...
                            onclick="EmailManagerSeguridad.generarEnviarPDFEvaluacion(${row.id})" 
...
function guardarObjetivoYSiguiente() {
  ...
  $.ajax({
    url: "/seguridad/objetivos_evaluacion/?campania="+ periodoARegistrar,
    type: "GET",
...
function eliminarObjetivo(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: `/seguridad/detalles_objetivos/${id}/`,
        type: "DELETE",
        headers: {
          "X-CSRFToken": $("[name=csrfmiddlewaretoken]").val(),
        },
...
function actualizarObjetivo() {
  ...
  $.ajax({
    url: `/seguridad/detalles_objetivos/${datosObjetivo.id}/`,
    type: "PUT",
    contentType: "application/json",
...
const EmailManagerSeguridad = {
  // Configuración del módulo específica para Almacén
  config: {
    endpoints: {
      evaluaciones: "/seguridad/objetivos_evaluacion/",
      detallesObjetivos: "/seguridad/detalles_objetivos/",
    },
    scripts: {
      competenciasPaths: [
        "/static/core/js/SEGURIDAD/DonLuis/rrhh_ev_competencias.js",
        
      ],
    },
    messages: {
      confirmation: {
        title: "Confirmación - SEGURIDAD",
      ...
      loading: {
        title: "Procesando - SEGURIDAD",
        text: "Generando PDF desde SEGURIDAD y preparando envío...",
      },
      errors: {
        invalidId: "No se pudo identificar la evaluación en SEGURIDAD",
    ...
    metadata: {
      ...
      version: "1.0.0",
      descripcion:
        "Gestión de correos para evaluaciones de objetivos - SEGURIDAD",
    },
  },
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Inicializar el módulo de gestión de correos electrónicos para SEGURIDAD
  EmailManagerSeguridad.inicializar();
