...
function calcularTotalPresupuesto_Capex() {
  ...
  if ($.fn.DataTable.isDataTable("#capexTable")) {
    ...
    data.forEach(function(row) {
      for (var mes in totalesMes) {
        totalesMes[mes].cantidad += parseFloat(row[mes + '_cantidad']) || 0;
        totalesMes[mes].precio += parseFloat(row[mes + '_precio']) || 0;
    ...
    meses.forEach(function(mes) {
      ...
      $(`#total_${mes}_cantidad`).text(totalesMes[mesLower].cantidad);
      $(`#total_${mes}_precio`).text(`S/. ${totalesMes[mesLower].precio.toFixed(2)}`);
    });
...
function initCapexTable() {
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate ? "/seguridad/seguridad_capex/" + formId + "/" : "/seguridad/seguridad_capex/",
 ...
 */
  ...
  $("#registerMaterialForm_capex").submit(function (e) {
    ...
    $.ajax({
      url: isUpdate
        ? "/seguridad/seguridad_capex/" + formId + "/"
        : "/seguridad/seguridad_capex/",
      type: isUpdate ? "PUT" : "POST",
      contentType: "application/json",
      data: JSON.stringify(jsonData),
      success: function (response) {
...
function editarProducto_capex(id) {
  $.ajax({
    url: "/seguridad/seguridad_capex/" + id + "/",
    type: "GET",
    success: function (data) {
