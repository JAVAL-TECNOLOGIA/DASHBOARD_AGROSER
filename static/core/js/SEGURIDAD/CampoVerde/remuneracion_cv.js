if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  function actualizarTotalSueldos() {
    ...
    $.ajax({
      url: "/seguridad/seguridad_sueldo_mensual_cv/?year=" + year,
      type: "GET",
      dataType: "json",
  ...
  function actualizarTotalSalarios() {
    ...
    $.ajax({
      url: "/seguridad/seguridad_salario_mensual_cv/?year=" + year,
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
  ...
  function initPresupuestoSueldos() {
    obtenerTasaCargaSocial().then(function () {
      ...
      tableSueldos = $("#sueldosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              dt.ajax.url("/seguridad/seguridad_sueldos_cv/?year=" + currentYear).load(function () {
        ...
        ajax: {
          url: "/seguridad/seguridad_sueldos_cv/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
    ...
    $("#formSueldoSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/seguridad/seguridad_sueldos_cv/${formId}/`
            : "/seguridad/seguridad_sueldos_cv/",
  ...
  function initPresupuestoSalarios() {
    obtenerTasaCargaSocial_salarios().then(function () {
      ...
      tableSalarios = $("#salariosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              // Recargar la tabla con el año correcto
              dt.ajax.url("/seguridad/seguridad_salarios_cv/?year=" + currentYear).load(function () {
                // Callback después de la recarga
                setTimeout(() => {
                  $(node).find("i").removeClass("fa-spin");
                }, 1000);
        ...
        ajax: {
          url: "/seguridad/seguridad_salarios_cv/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/seguridad/seguridad_salarios_cv/${formId}/`
            : "/seguridad/seguridad_salarios_cv/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
  ...
  function eliminarSueldo(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/seguridad/seguridad_sueldos_cv/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
  ...
  function eliminarSalario(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: `/seguridad/seguridad_salarios_cv/${id}/`,
          type: "DELETE",
          success: function (response) {
            if (response.status === "success") {
              Swal.fire("¡Eliminado!", response.message, "success");
