if (typeof window.presupuestoMaterialesInitialized === "undefined") {
  ...
  function actualizarTotalSueldos() {
    ...
    $.ajax({
      url: "/sst/sst_sueldo_mensual_ajs/?year=" + year,
      type: "GET",
      dataType: "json",
  ...
  function actualizarTotalSalarios() {
    ...
    $.ajax({
      url: "/sst/sst_salario_mensual_ajs/?year=" + year,
      type: "GET",
      dataType: "json",
      success: function (response) {
        const totalAnual = response.data.find(
  ...
  function initPresupuestoSueldos() {
    obtenerTasaCargaSocial().then(function () {
      ...
      tableSueldos = $("#sueldosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              dt.ajax.url("/sst/sst_sueldos_ajs/?year=" + currentYear).load(function () {
        ...
        ajax: {
          url: "/sst/sst_sueldos_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
  ...
  function initPresupuestoSalarios() {
    obtenerTasaCargaSocial_salarios().then(function () {
      ...
      tableSalarios = $("#salariosTable").DataTable({
        ...
        buttons: [
          {
            ...
            action: function (e, dt, node, config) {
              ...
              // Recargar la tabla con el año correcto
              dt.ajax.url("/sst/sst_salarios_ajs/?year=" + currentYear).load(function () {
                // Callback después de la recarga
                setTimeout(() => {
                  $(node).find("i").removeClass("fa-spin");
                }, 1000);
        ...
        ajax: {
          url: "/sst/sst_salarios_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
          dataSrc: "data",
        },
        columns: [
          { data: "id", title: "ID", visible: false },
    ...
    $("#formSalario")
      ...
      .on("submit", function (e) {
        ...
        $.ajax({
          url: isUpdate
            ? `/sst/sst_salarios_ajs/${formId}/`
            : "/sst/sst_salarios_ajs/",
          type: isUpdate ? "PUT" : "POST",
          contentType: "application/json",
          data: JSON.stringify(jsonData),
          success: function (response) {
