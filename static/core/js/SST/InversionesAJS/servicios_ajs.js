...
function initServiciosTable() {
  ...
  var table = $("#serviciosTable").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          ...
          dt.ajax.url("/sst/sst_servicios_ajs/?year=" + currentYear).load(function () {
    ...
    ajax: {
      url: "/sst/sst_servicios_ajs/?year=" + ($('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear()),
      dataSrc: "data",
    },
...
function actualizarTotalWidget() {
  var yearVal = $('#filtroAnioPresupuesto').val() || 'CAMP' + new Date().getFullYear();
  $.ajax({
    url: `/sst/sst_servicios_totals_ajs/?year=${yearVal}`,
    type: "GET",
...
$("#registerMaterialForm_servicios").submit(function (e) {
  ...
  const ajaxConfig = {
    url: isEdit
      ? `/sst/sst_servicios_ajs/${id}/`
      : "/sst/sst_servicios_ajs/",
